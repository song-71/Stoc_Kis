...
> 아래에 나오는 파일명 중 ws_realtime_trading은 현재 ws_realtime_trading.py로 이름을 바꾸었음. 
  DB-2는 기존 ws_realtime_trading 중 wss 데이터 안정적 수신을 위한 백업파일이었음.. 현재는 wss 데이터 수신이 안정적이므로 DB-2는 고려하지 않아도 됨.
  아래의 구조로 여러 사람과 계좌를 적용하능하도록 리팩토링 해 주되, 각 계좌의 거래 적용여부, 일일 거래 자본 한도, 계좌별 제외금액 등을 입력 관리하도록 해줘..

---
1. 목표 구조 요약
항목	내용
코드베이스	ws_realtime_trading 한 파일로 통합 (같은 전략 로직)
...
| 코드베이스 | ws_realtime_trading 한 파일로 충분 |
...
## 4. ws_realtime_trading, DB-2 관련 (참고)

- **ws_realtime_trading, DB-2**: 정해진 종목 웹수신 vs 수시 변동 웹수신, 에러 시 백업 데이터 확보용으로 사용
- **실제 운용**: 앞으로는 ws_realtime_trading만으로 충분
- **두 개로 나눠 본 이유**: 한 계정 용량 초과 시 다른 계정으로 전환 가능함을 확인
- **향후**: ws_realtime_trading 로직을 다른 사람 계정이 추가되어도 동시에 적용
...
3. ws_realtime_trading 코드를 계좌 순회 루프로 전환
