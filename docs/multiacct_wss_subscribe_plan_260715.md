...
## 9. 확정 사항 (2026-07-15 사용자 컨펌)

1. **a2 = 구독 전용, 매매는 a1만.** → `a2.trade_enabled=False` 유지. **a2 연결은 시세(ccnl/exp/H0STMKO0)만**, 체결통보(H0STCNI0)·주문 없음. 자본 분리 없음. (§6-3 리스크 소멸: a2는 CNI0 슬롯 불필요 → 40슬롯 전부 시세용.)
2. **범위 = 우선 2연결(a1+a2).** 최대 80슬롯. 코드는 N계좌로 일반화하되, 실동작 검증은 2연결까지. a3+는 appkey 확보 시 config 추가만으로 확장.
3. **구독 ≠ 매매(재확인).** 종목을 어느 연결로 구독하든 주문은 기존 REST 계좌별 경로. 매매는 이 리팩터와 독립.

### 배정 규칙 확정본
- a1 연결 = `H0STCNI0(1, a1만) + H0STMKO0 + 보유·VI·전일상한가 종목데이터` 우선.
- a2 연결 = **오버플로 전용**(당일 상한가 도달 종목 중 a1 40슬롯 초과분). CNI0 없음.
- 한 종목은 정확히 한 연결에만(중복 구독 금지). 청산 시 슬롯 반납·회수 배정.

> **다음 단계: 이 방향대로 착수 대기.** 사용자의 최종 "시작" 지시가 오면 브랜치 `feature/multiacct-wss-subscribe` 에서 구현을 시작한다. 그 전에는 프로덕션 코드를 수정하지 않는다.
