...
## [2026-07-14] e82938a
- **Category**: fix
- **Title**: VI 감지 사각지대 해소 - H0STMKO0 관찰구독 동적 슬롯cap + REST "돌연 무음→VI 의심" 09:00 조기감지
- **Files**: `ws_realtime_trading.py`, `rules/ws_realtime_trading_logic.md`, `ws_monitoring_research_260713.md`(신규)
- **Changes**:
  1. **사고 경위(260713 09:04:48)**: 002880(디와이에이) 상한 VI 발생 시 실시간체결(H0STCNT0)이 약 2분12초간 끊기고, VI 중 나와야 할 예상체결가(H0STANC0)가 전혀 수신되지 않음. VI 자체를 감지하지 못해 실시간체결→예상체결가 재전환 로직이 발동되지 않은 것이 근본원인. 감지 경로 A(H0STMKO0 장운영코드)와 경로 B(REST 무데이터 감시기의 "돌연 무음→VI 의심" 판정)가 개장 직후(09:00~09:10) 모두 사각지대였음.
  2. **경로 A 개선 — H0STMKO0 관찰구독 슬롯 동적화**: 기존 고정 5개(`H0STMKO0_MAX_SLOTS`)라 002880이 관찰구독 대상에서 밀려 애초에 미구독 상태였음. `_h0stmko0_slot_cap()` 신설(약 8606번 줄): 가용슬롯 = MAX_WSS_SUBSCRIBE(40) − H0STCNI0(1) − 현재 데이터구독종목수(len(codes)) − 예비마진 H0STMKO0_DATA_RESERVE(5), 최저 H0STMKO0_MIN_SLOTS(5) 보장. 호출부 2곳 갱신: `_mkstatus_sub_add`(약 8640번 줄, 가용계산+cap 로그), open_map 재구성(약 13719번 줄, `if len(mkstatus_target) > _mko_cap`).
  3. **경로 B 개선 — REST 무음감시 게이트 09:10 전용 → 09:00 이후 상시**: 기존엔 09:10 이후에만 동작해 09:00~09:10 구간의 VI를 놓쳤음. `_price_watchdog_loop`의 09:00 가드 직후(약 8864번 줄)에 독립 감지 블록 신설, 09:00~15:30 전 구간 상시 동작(섹션 A의 조기 return과 무관). 예상체결가연장(_vi_delayed_codes)·예상체결가상태(_vi_exp_sub_ts)·09:00~09:02 거래정지(_halted_codes) 종목은 제외. 판정조건: 평균틱간격<5초 & 유휴시간≥max(10, 평균*3), 종목별 10초 간격으로 재검사. 기존 (C) 섹션의 중복 "돌연 무음" 블록(약 9004번 줄)은 제거.
  4. **문서**: `rules/ws_realtime_trading_logic.md` §2에 경로 A/B 트리거·커버리지 설명 추가, §6-1 "WSS 슬롯 배분" 신설, 맨 아래 260714 변경 이력 행 추가. `ws_monitoring_research_260713.md` 신규 — 사고 진단 + 260714 구현 내용 요약.
- **Impact**: 개장 직후(09:00~09:10) 발생하는 VI도 두 경로 중 하나로 감지 가능해짐 — H0STMKO0 관찰구독이 종목수 변동에 맞춰 슬롯을 재배분해 관찰대상 누락 가능성이 줄고, REST 감시기가 09:00부터 상시 동작해 개장 직후 VI도 놓치지 않음. 002880 사고와 같은 실시간체결 장기 단절(예상체결가 미전환) 재발 가능성 감소.
