...
def inquire_time_itemchartprice(
    ...
    if res.isOK():
        ...
        output2 = getattr(body, "output2", []) or []

        # output (object) -> DataFrame (1행)
        if isinstance(output, list):
            output = output[0] if output else {}
        output1_data = pd.DataFrame(output, index=[0])
        
        # output2 (array) -> DataFrame (여러행)
        output2_data = pd.DataFrame(output2)
        
        return output1_data, output2_data
    else:
        res.printError(api_url=API_URL)
        return pd.DataFrame(), pd.DataFrame()


API_URL_DAILY = "/uapi/domestic-stock/v1/quotations/inquire-time-dailychartprice"
TR_ID_DAILY = "FHKST03010230"
...
def inquire_time_dailychartprice(
    ...
    fid_fake_tick_incu_yn: str = "",
) -> Tuple[pd.DataFrame, pd.DataFrame]:
    """
    주식일별분봉조회 API. 특정 날짜 지정 가능.
    반환: (output1, output2) - output1은 일부 필드만 있을 수 있음.
    """
    ...
    params = {
        ...
        "FID_INPUT_HOUR_1": fid_input_hour_1,
        "FID_PW_DATA_INCU_YN": fid_pw_data_incu_yn,
        "FID_FAKE_TICK_INCU_YN": fid_fake_tick_incu_yn or "",
    }
    res = ka._url_fetch(API_URL_DAILY, TR_ID_DAILY, "", params)
    if res.isOK():
        body = res.getBody()
        output = getattr(body, "output", []) or []
        output2 = getattr(body, "output2", []) or []
        output1_data = pd.DataFrame(output, index=[0]) if output else pd.DataFrame()
        output2_data = pd.DataFrame(output2)
        return output1_data, output2_data
    else:
        res.printError(api_url=API_URL_DAILY)
        return pd.DataFrame(), pd.DataFrame()