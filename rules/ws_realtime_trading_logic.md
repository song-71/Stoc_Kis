...
## 5. 호가(orderbook) 레코더 — 핵심 검증됨 + 260715 비활성 + 260722 트리거 통합
- **[260722] 눌림목 트리거 오더북 메인 통합(신규 경로).** 구 레코더(별도 프로세스+공유메모리 버스)와 **별개로**, str2 눌림목이 **상한가 이탈 종목만** `on_result` 에서 H0STASP0 를 직접 구독·수신해 `_str2_ob_latest`(종목별 최신 1건)에 유지한다(`STR2_OB_ON_TRIGGER=True`). `_get_orderbook` 이 이 스토어를 **우선** 반환(→ str2 SIMUL 체결 판정). 구독/해제는 `_str2_ob_worker` 가 `asking_price_krx("1"/"2")` 로 수행. 구 `ORDERBOOK_ENABLED`(레코더/버스) 는 여전히 False 이며 이 신규 경로와 무관(독립). 상세는 변경 이력 260722 참조.
...
| 날짜 | 변경 | 관련 섹션 | 코드 위치 |
|---|---|---|---|
| 2026-07-23 | **다계좌 WSS a3(3계좌) 추가 + 눌림목 깊이하한 4% + 레거시 TOP5 정리.** ① **a3 격리 연결**(`_setup_ka3`/`run_ws_forever_a3`/`_a3_active`/`_a3_share`, a2 미러) — a3=순수 오버플로 데이터 전용(MKO·CNI0 없음, 전용 appkey cano 44879705). `_partition_a1_a2` 2분할→3분할, 예산 항상계산+미연결분 a1 흡수. 오더북 배정(`_str2_ob_pick_acct`/`_str2_ob_worker`) a3 분기. 데이터 슬롯 ~74→~113(급등장 유니버스 79 오버플로 해소). **주의: 오버플로는 유니버스(codes) 크기 기준 판정 — 15:20 exp 전환해도 유니버스 79 유지되면 경고 지속(실시간 유지 아님).** ② **진입 깊이 하한 2%→4%**(전 기간 101일 스윕 최적, 복귀시점 `_depth` 게이트 0.02→0.04, `DEPTH_FLOOR_PCT`). ③ **레거시 정리**: `TOP5_ADD`→`CLOSING_SELECT_ENABLED`(종가매매 15:19선정/15:20전환 게이트, False) 개명, 죽은 옛 top30 동적구독 루프 129줄 제거(현 20%↑ 편입은 UPLIMIT_WATCH 담당). 동작 보존. | §2·§5 | `_setup_ka3`·`run_ws_forever_a3`·`_partition_a1_a2`·`_a3_data_budget`·`CLOSING_SELECT_ENABLED` |
| 2026-07-22(3) | **마감 후 검증본 대조 감사 → 차이 수정(사용자 요청 "시뮬레이션과 동일").** ① **매도 로직 검증본 일치**: 구 str2 의 sell_ready·ma500gap·지정가매도(sell_limit_place_ok/sell_order_resolve)·손절/트레일 제거 → **상한가재도달 / ma500<ma2000 데드(golden) / 15:18 EOD** 3청산만(sweep_exec_sim `exit_from` 과 동일). ② **오더북 쓰레기행 제거**: H0STASP0 프레임마다 딸려오는 '가격을 code로 가진 all-zero 행'(실측 47%) 필터(`askp1>0 or bidp1>0`) — 옛 레코더의 `allowed` 필터에 대응. ③ **CSV→parquet(zstd)**: `_flush_str2_ob_save` 가 parts parquet 저장, `_merge_str2_ob_parts` 가 종료 시 일별 병합(`data/wss_data/orderbook/{ymd}_orderbook.parquet`). ④ **장마감(15:30) 후 오더북 구독 중단·해제**(`STR2_MKT_CLOSE_SEC`) — 시간외 호가 캡처 방지(실측: 시간외엔 KIS가 무변화 동일 스냅샷 반복 전송). ⑤ **중복 스냅샷 제거**(`_str2_ob_last_sig`): 호가 변화 있을 때만 저장. ⑥ **원본 캡처**(`_str2_ob_raw_capture`, `raw_diag/`): 하루 첫 400건 전체 59컬럼 원본 저장 — '예상체결가-as-code all-zero 행'이 KIS 원본인지 파싱(WSS 59컬럼 청킹)인지 진단·민원 근거용. **미해결/검토대상(사용자 확인 필요)**: (a) 지정가 체결모델 — 라이브는 실시간 orderbook(askp1≤P), 검증본은 과거 dv=5 물량(의도적 차이=실측 검증). (b) 데드 청산 체결가/수수료(검증본 dntk(ma2000)·지정가 vs 라이브 시장가). (c) MAXE=5 는 검증본 기본값이나 독립 스윕최적화 아님. (d) EOD 15:18 vs 검증본 장마감. | §5·§7 | 매도부·`_str2_ob_ingest`(필터)·`_flush_str2_ob_save`/`_merge_str2_ob_parts`(parquet)·`STR2_MKT_CLOSE_SEC` |
| 2026-07-22(2) | **str2 진입 로직을 검증본과 완전 일치로 재작성 + 오더북 디스크 저장 + 진단로그(장중 점검서 결함 발견 → 수정).** ⚠️ 이전 260722(1)의 str2 SIMUL 은 **b3 추세게이트(ma50<ma300)가 갓 상한가 종목엔 항상 False → 매수 0건**이었고, 오더북도 **메모리 최신만 보관·디스크 미저장**이었음. 수정: **① 진입 재작성(sweep_exec_sim.py 정합)**: 상한가 도달→`armed`, ma10<bb_lo 연속 `STR2_ESC_MIN=3`회→`escaped`, **복귀(ma10≥bb_lo & ma10>ma50) + 깊이(상한가-현재가)/상한가 0.5%↑** 시 진입, 진입 후 `armed=False`(상한가 재도달 시 재무장), 종목당 `STR2_MAXE=5`회. 상한가 복귀 시 미체결 지정가 취소. **② 라우팅**(`_str2_place_buy(depth)`): 깊이≤2%(`STR2_SHALLOW`)=지정가(ma10 최저) / 깊이>2% & 투입액>4천만(`STR2_SIZE_LIM_MAX`)=시장가 스윕. **③ 오더북 디스크 저장**: `_str2_ob_ingest`가 프레임을 버퍼→`data/wss_data/orderbook/{YYMMDD}_orderbook.csv` 배치 저장(2000건/30초 flush, `_flush_str2_ob_save`). **④ 진단로그**: `[str2_이탈]`(밴드하단 이탈확정)·`[str2_진입]`(복귀매수)·오더북 저장건수. Str2State 필드 armed/esc_count/escaped/entries 추가. | §5·§7 | `STR2_ESC_MIN`·`STR2_MAXE`·`STR2_SHALLOW`·`STR2_SIZE_LIM_MAX`·`_str2_ob_ingest`·`_flush_str2_ob_save`·`_str2_place_buy(depth)` / `ws_realtime_tr_str2.py`(Str2State) |
| 2026-07-22 | **str2 상한가 눌림목 검증본 정합 + 모의투자(SIMUL) 상시 ON + 눌림목 오더북(H0STASP0) 메인 통합 (로컬 검증본과 동기화, 사용자 요청).** ⚠️ **실거래는 계속 OFF**(`STR2_ENABLED=False`), **`STR2_SIMUL_MODE=True`(기본 ON)** 로 트리거·주문·체결·청산을 전부 `_append_ledger` 장부에만 기록(실주문·취소 미발주). **① 검증본 정합(str2)**: 손절/트레일 제거(`sell_stop_trail`→항상 `(False,"")`); 데드크로스 `ma10<ma500`→**`ma500<ma2000`**(정배열 `golden_seen` 래치 후 하향교차, ma2000 워밍업 전 구로직 폴백); 지정가 매수가 `직전50틱 min(bidp1)`→**`min(ma10)`**(`_str2_ma10_buf`); **눌림목(b3)만**(`STR2_PULLBACK_ONLY=True`→b1 장전하락·b2 시초시장가 비활성); **상한가 재도달 즉시 지정가 매도**(매도 최상단); **진입 깊이 하한 0.5%**(`depth_floor_ok`, `DEPTH_FLOOR_PCT=0.005`, 상한가 대비 눌림 저점 `pullback_low` — 검증본 결론은 2%하한 우월이나 **모의투자 데이터 수집 위해 0.5%로 낮춰 0.5~2% 얕은구간도 장부 기록** → 나중에 제외/포함 성과 재분석); 매도 최종=상한가재도달/`ma500<ma2000`데드/15:18 EOD. **② 오더북 메인 통합**: 별도 프로세스·공유메모리 버스 없이, **상한가 이탈(미보유) 시 그 종목만 H0STASP0 구독**→**보유 중이면 청산까지 유지**, 상한가 복귀(미보유)·청산·신호없이 `STR2_OB_HOLD_SEC=300`초 경과 시 자동 해제. `on_result` 에 H0STASP0 분기(`_str2_ob_ingest`→`_str2_ob_latest` 최신1건)·`_get_orderbook` 이 이를 우선 참조(SIMUL 체결 판정). 구독/해제 send_request 는 전용 워커(`_str2_ob_worker`, tick·이벤트루프와 분리) 가 수행 — **a1 여유 시 a1, a1 만석 시 a2 오버플로**(`_str2_ob_pick_acct`; `_a1/_a2_data_budget` 이 오더북 슬롯을 데이터 예산에서 차감해 40 초과 방지). a2 전송은 `_a2_req(asking_price_krx)`(a2 approval_key). **재접속 견고화**: a1·a2 (재)접속 시 `_str2_ob_reset(acct)` 로 로컬 구독상태 비움 → 트리거/보유가드가 재구독. 보유 중 오더북 유실 시 청산 전까지 재구독 보장. **③ SIMUL 게이트 확장**: `_check_str2_from_tick` 호출·상태저장/복원·carry 익일매도 게이트를 `STR2_ENABLED or STR2_SIMUL_MODE` 로. **미검증(라이브)**: 오더북 동적 구독의 재접속 재구독·슬롯(현재 a1) 관측 필요. **검증법**: 장부(ledger) + `data/wss_data` 체결틱 + H0STASP0 로그 대조(오프라인). | §5·§7 | `STR2_SIMUL_MODE`·`STR2_PULLBACK_ONLY`·`STR2_OB_ON_TRIGGER`·`_str2_ob_ingest`·`_str2_ob_subscribe`·`_str2_ob_worker`·`_get_orderbook` / `ws_realtime_tr_str2.py`(`sell_stop_trail`·`sell_deadcross`·`depth_floor_ok`·`Str2State`) |
| 2026-07-20 | **당일 상한가 감시 하한 28→20% 완화 + 30건 벽 돌파 + 두 계좌 슬롯합계 상한관리(사용자 요청).** ①하한 `UPLIMIT_WATCH_RATE_MIN "28"→"20"`. ②등락률순위 API(FHPST01700000)는 **응답 30건 하드캡·연속조회 불가** → `UPLIMIT_WATCH_RATE_BANDS`=[(20,24),(24,27),(27,30)] 밴드별 조회 후 합집합(`_fetch_uplimit_band` 신설, `_fetch_uplimit_candidates` 는 밴드 순회 dedup)으로 30건 초과 발굴. ③`_partition_a1_a2` 에 `_a2_data_budget()`(=40−H0STMKO0) 추가, **a1+a2 슬롯 합계 초과 시 최저우선순위(당일 20%↑ 꼬리) 절단** — 보유·VI·전일상한가는 우선순위 앞쪽이라 절대 미절단. **주의**: 단일 밴드 자체가 30건을 넘는 극단(상한가 대량일)엔 그 밴드에서 초과분 누락 가능. | §6-1·§0 | `:119~126` `UPLIMIT_WATCH_RATE_BANDS`·`_fetch_uplimit_band`·`_fetch_uplimit_candidates`·`_partition_a1_a2`·`_a2_data_budget` |
