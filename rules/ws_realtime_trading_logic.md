...
| 날짜 | 변경 | 관련 섹션 | 코드 위치 |
|---|---|---|---|
| 2026-07-15 | **다계좌 WSS 구독 리팩터 S1 — orderbook 레코더 비활성.** `ORDERBOOK_ENABLED=True→False`(:109). 호가 전략 당분간 미사용 + a2 계좌를 다계좌 구독 연결로 전용하기 위해 a2 orderbook subprocess 기동 제거. 후속 단계(S2~): a2 격리 모듈 사본 연결(H0STMKO0 + 오버플로 데이터), 구독 총괄관리자(전일상한가 ∪ 당일28%↑ sticky). 설계: `docs/multiacct_wss_subscribe_plan_260715.md`. | §5·§0 | `:109`, `_start_orderbook_recorder:14055` |
