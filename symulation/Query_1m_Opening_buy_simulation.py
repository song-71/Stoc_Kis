"""
상한가 도달 종목 익일 시가 매수 → 트레일링 스톱 매도 시뮬레이션
Query_1m_Opening_buy_simulation.py

전략:
  1) 1d 데이터에서 당일 종가가 상한가(전일종가 * 1.30, 호가단위 적용)인 종목 선정 (상한가 마감)
  2) 해당 종목을 다음 거래일 시가(open)에 매수
  3) 같은 거래일 1m 데이터로 트레일링 스톱 시뮬레이션:
     - 최고가가 매수가 대비 +5% 이상 → 트레일링 스톱 활성화
     - 트레일 스톱 가격 = buy_price × (1 + floor(peak_ret / 1%) × 1%)
       즉, 매수가 기준으로 1% 단위 내림한 수익률을 매도선으로 설정
     - 이후 최고가 갱신 시 1%씩 상향 (6%, 7%, ... 도달 시 트레일 스톱도 상향)
     - 저가가 트레일 스톱 이하 → 트레일 스톱 가격에 매도
     - 15:18까지 미발동 → 15:18 종가에 매도

     [트레일링 스톱 예시] 매수가 10,000원
       최고가 10,500(+5.0%) → 트레일스톱 10,500원(+5%)
       최고가 10,730(+7.3%) → 트레일스톱 10,700원(+7%)
       최고가 11,200(+12%)  → 트레일스톱 11,200원(+12%)
       → 저가가 11,200원 이하 터치 시 11,200원에 매도
     ※ "고점 대비 -N% 하락" 방식이 아닌, "매수가 대비 +N% 선 유지" 방식
  4) 수익률 계산 후 일자별/월별 집계 CSV 저장

사용법:
  python Query_1m_Opening_buy_simulation.py --start 20250103 --end 20250131
"""

# ── 기본 옵션 ──
DEFAULT_START = "20260201"
DEFAULT_END   = "20260325"
...
CLOSE_GAP_THRESHOLDS = [0.00]  # 상한가 대비 종가의 괴리율 임계값 (0%=상한가 마감, 필요시 0.03, 0.05 등 추가)
STOP_LOSS_LEVELS = [0.0, 0.05, 0.10]  # 스톱로스 수준 (0.0=매수가, 0.05=-5%, 0.10=-10%)

# 트레일 모드: type="buy_based"(매수가 기준 +N%), type="peak_drawdown"(고점 대비 -N%)
TRAIL_MODES = [
    {"name": "트레일 +5%", "type": "buy_based", "activate": 0.05, "step": 0.01},
    {"name": "고점대비 -3%", "type": "peak_drawdown", "drawdown": 0.03},
    {"name": "고점대비 -5%", "type": "peak_drawdown", "drawdown": 0.05},
]
...
def _find_upper_limit_stocks(con: duckdb.DuckDBPyConnection, date_str: str) -> pd.DataFrame:
    ...
    df = df[df["close"] >= df["upper_limit"]].copy()
...
def _get_next_day_open_prices(con: duckdb.DuckDBPyConnection, next_date_str: str, codes: list[str]) -> dict[str, float]:
    """다음 거래일 1d 데이터에서 시가(open)를 조회"""
    codes_sql = ",".join(f"'{c}'" for c in codes)
    df = con.execute(f"""
        SELECT symbol, open
        FROM read_parquet('{PARQUET_1D}')
        WHERE date = '{next_date_str}'
          AND symbol IN ({codes_sql})
          AND open > 0
    """).df()
    if df.empty:
        return {}
    df["symbol"] = df["symbol"].astype(str).str.zfill(6)
    return dict(zip(df["symbol"], df["open"].astype(float)))


def _get_available_1m_dates(con: duckdb.DuckDBPyConnection) -> set[str]:
    """S3에 실제 존재하는 1m 날짜 목록 조회"""
    try:
        df = con.execute(
...
def _simulate_trailing_stop(
    ...
    trail_mode: dict | None = None,
    available_1m_dates: set[str] | None = None,
) -> dict[str, dict]:
    """
    다음 거래일 1m 데이터(S3)로 트레일링 스톱 시뮬레이션.
    trail_mode: TRAIL_MODES 중 하나. None이면 기본(buy_based +5%).
    """
    if trail_mode is None:
        trail_mode = TRAIL_MODES[0]

    # S3에 해당 날짜 데이터가 없으면 skip
    if available_1m_dates is not None and next_date_str not in available_1m_dates:
        print(f"  [skip] S3 1m 데이터 없음 ({next_date_str})")
        return {}
    ...
    is_peak_drawdown = trail_mode["type"] == "peak_drawdown"
    drawdown_pct = trail_mode.get("drawdown", 0.05) if is_peak_drawdown else 0.0
    trail_activate = trail_mode.get("activate", TRAIL_ACTIVATE)
    trail_step = trail_mode.get("step", TRAIL_STEP)
    ...
    for code, grp in df.groupby("code"):
        ...
        stop_loss_price = buy_price * (1.0 - stop_loss) if stop_loss is not None else 0.0
        sold = False
        stop_loss_hit = False
        sell_price = 0.0
        sell_time = ""
        ...
        for i in range(len(grp)):
            ...
            if is_peak_drawdown:
                # 고점 대비 하락률 방식: 최고가 대비 drawdown_pct 이탈 시 매도
                if peak_price > 0:
...
def main() -> None:
    ...
    print(f"[{start_ts}] 상한가 도달 종목 익일 시가 매수 시뮬레이션 시작")

    parser = argparse.ArgumentParser(description="상한가 도달 종목 익일 시가 매수 시뮬레이션")
    ...
    try:
        ...
        # === Phase 1: 상한가 도달 종목 + 다음 거래일 시가 매수가 수집 ===
        ...
        for date_str in trading_dates:
            ...
            # 다음 거래일 시가(open)를 매수가로 조회
            open_prices = _get_next_day_open_prices(con, next_date_str, candidate_codes)

            buy_info = {}
            for code in candidate_codes:
                op = open_prices.get(code, 0.0)
                if op > 0:
                    buy_info[code] = op
        ...
        for tm in TRAIL_MODES:
          for sl in STOP_LOSS_LEVELS:
            sl_label = "매수가 이탈즉시 매도" if sl == 0.0 else f"스톱로스 -{sl*100:.0f}%"
            combo_label = f"{sl_label} / {tm['name']}"
            print(f"\n{'='*60}")
            print(f"  [{combo_label}]")
            print(f"{'='*60}")
            ...
            for date_str, next_date_str, buy_info, code_row_map in stock_data:
                trail_results = _simulate_trailing_stop(con, layout, next_date_str, buy_info, stop_loss=sl, trail_mode=tm, available_1m_dates=available_1m_dates)
            ...
            # 상세 CSV
            tm_suffix = tm["name"].replace(" ", "").replace("+", "p").replace("-", "m").replace("%", "pct")
            sl_suffix = f"sl{int(sl*100):02d}"
            detail_path = BASE_DIR / f"opening_buy_detail_{tm_suffix}_{sl_suffix}.csv"
        ...
        summary_path = BASE_DIR / f"opening_buy_summary_{args.start}_{args.end}.csv"
        ...
        if all_summary_rows:
            ...
            comp_cols = ["트레일모드", "스톱로스", "괴리율", "총종목수", "수익", "손실", "승률", "평균수익률"]
            comp_align = {c: "right" for c in comp_cols}
            comp_align["트레일모드"] = "left"
