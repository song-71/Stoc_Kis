...
사용법:
  ...
  python ws_merge_wss_parts.py 1        # TR (수신저장) 만 병합
...
병합 방식:
  ...
  - 병합시 중복 데이터 제거 추가(26.3.2. claude)
"""
...
# ── 옵션 설정 ────────────────────────────────────────────────────────
#   0 : 전체 일괄 병합
#   1 : TR (수신저장) 만 병합(data/wss_data 폴더)
...
# ── 병합 대상 경로 설정 ──────────────────────────────────────────────
#   ws_realtime_trading.py → data/wss_data
...
def _merge_date_group(
    ...
    if "recv_ts" in df.columns:
        try:
            ...
            ts_range = ""
            try:
                ts_range = f", recv_ts 범위: {df['recv_ts'].min()} ~ {df['recv_ts'].max()}"
            except Exception:
                pass
            logger.warning(
                f"{base_msg} 정렬/중복제거 실패 — 종목: {stock_names}{ts_range}, "
                f"rows: {len(df):,}, error: {e}"
            )
...
def _resolve_target_date() -> str:
    """TARGET_DATE 옵션을 해석하여 YYMMDD 문자열 반환. 빈 문자열이면 오늘 날짜."""
    if TARGET_DATE:
        return TARGET_DATE
    return datetime.now(KST).strftime("%y%m%d")


def _merge_target(target: dict, filter_date: str = "") -> None:
    """하나의 타겟(parts 폴더)에 대해 날짜별 그룹핑 후 병합.
    filter_date가 주어지면 해당 날짜만 병합."""
    ...
    if filter_date:
        if filter_date in date_groups:
            ...
            logger.info(f"[{target['label']}] 날짜 필터 적용: {filter_date}")
        else:
...
def main() -> None:
    ...
    for idx in selected:
        ...
        print(f"{'─' * 50}")
        _merge_target(t, filter_date=filter_date)
