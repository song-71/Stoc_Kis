...
| **260519 (어제)** | **5회** (1 초기 + 4 외부 재시작) | 08:28:03(초기) · 09:40:51(SIGKILL) · 10:13:22 · 10:14:20 · 13:44:35(SIGTERM) |
...
**[정정] 어제 장중 재시작 4건은 전부 외부 종료(배포/수동 재시작)였고 WSS 장애가 아니다.**
- 직전 로그가 `[shutdown] signal=15`(SIGTERM, 외부 종료) 3건: 10:11:58, 10:13:38, 13:44:27 → graceful shutdown → `os._exit(0)` → runner 재기동.
- 09:40:51 1건은 signal 로그조차 없음 — 직전이 정상 데이터 수신(`09:40:51.037 [체결가 수신]`)이고 0.68초 뒤 새 프로세스 기동, 에러/traceback 0건 → 건강한 프로세스를 외부에서 SIGKILL(kill -9)한 것으로 추정.
- **어제 `자기보호 종료(self-stop)/exit=2` 0건 — WSS 결함 재시작은 단 한 건도 없었다.** 08:50 핸드셰이크 storm 도 재시작 없이 내부 회복.
- 260519 는 코드 배포일(57a1405)이라 장중 재기동 4건은 정상적인 배포 재시작.
- **결론: 오늘·어제 모두 WSS 결함 재시작 0건.** (앞선 "어제가 더 불안정" 표현은 배포 재시작을 장애로 오인한 것이라 정정.)
