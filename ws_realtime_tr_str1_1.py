"""
Str1 실전 실시간 매도 전략 + 상한가 근접 매수 전략 모듈 (v_1)
...
== 신규 전략 (v_1, 문서: docs/01. up_limit_buy_str_260420.md) ==
- 상한가 근접 매수: 25~28% 구간 다중 필터 AND 통과 시 매수
...
== 신규 전략 (v_1, 문서: docs/02. Top30_str.md Part B) ==
- 종가매수 폭락 방지 필터
...
※ 이 모듈은 순수 전략 판단 함수만 포함 (API 호출·파일 I/O 없음).
   실제 주문·상태 관리는 ws_realtime_trading_1.py 에서 수행.
"""
