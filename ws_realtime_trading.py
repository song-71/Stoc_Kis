...
# Str1 가중평균 하락 비교 시 슬리피지 적용 (0.8% = 0.008, 0이면 비적용)
STR1_SLIPPAGE_RATE = 0.008
...
def _check_str1_sell_conditions(
    ...
    for df_code in df_tmp.partition_by(code_col, maintain_order=True):
        ...
        if is_premarket:
            ...
            bidp1 = 0.0; wghn = 0.0; oprc = 0.0; prdy_v = 0.0; stck_prpr = 0.0
            try:
    ...
    if (is_premarket or is_regular) and _premarket_watch:
        _print_premarket_monitor(newly_triggered, now_ts, is_premarket=is_premarket)


def _print_premarket_monitor(newly_triggered: list[str], now_ts: float, is_premarket: bool = True) -> None:
    """
    Str1 포지션 모니터링 출력.
    ...
    for code in newly_triggered:
        ...
        prefix = "🔴 [장전매도조건 진입]" if is_premarket else "🔴 [매도조건 진입]"
        msg = _fmt_premarket_alert_line(code, w, prefix=prefix)
        sys.stdout.write(f"\n{msg}\n")
        sys.stdout.flush()
        _notify(msg, tele=True)
        logger.info(msg)
    ...
    if dtime(8, 29) <= nt < dtime(9, 0):
        header_label = "[장전 예상체결가 모니터링]"
    elif dtime(15, 20) <= nt < dtime(15, 30):
        header_label = "[장마감 예상체결가 모니터링]"
    else:
        header_label = "[Str1 포지션 모니터링]"
    header = f"{'='*60}\n{header_label} {now_str}"
    lines   = [header]
    if sell_lines:
...
def _get_str1_status_label(code: str) -> str:
    ...
    if code in _single_price_codes:
        return "57/59"
...
def _fmt_premarket_alert_line(code: str, w: dict, prefix: str = "") -> str:
    ...
    price_label = "예상가" if stat in ("57/59", "57(단일가)", "59(단일가)") else "현재가"
    parts = [name]
    parts.append(f"[{stat}]")
