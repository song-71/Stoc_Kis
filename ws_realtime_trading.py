...
def _str1_sell_worker() -> None:
    ...
    while not _stop_event.is_set() or not _str1_sell_queue.empty():
            ...
            try:
                ...
                with _str1_sell_state_lock:
                    ...
                    _str1_sell_state[code] = {
                        **st,
                        "oca_ordered": False,
                        "oca_cancel_requested": False,
                        "ordno": "",
                    }
                    _oca_cancelled_log.append((code, name, reason))
                ...
                logger.error(f"[str1_sell] 동시호가매도 취소실패 {code}: {e}")
                _notify(f"{ts_prefix()} [str1_sell] 동시호가매도 취소실패 {name}({code}) err={e}", tele=True)
                with _str1_sell_state_lock:
                    if code in _str1_sell_state:
                        _str1_sell_state[code]["oca_cancel_requested"] = False
            continue
        ...
        try:
            ...
            is_oca = reason.startswith("str1_시초가하락_예상")
            sell_label = "스톱지정가매도" if is_stop_limit else ("동시호가매도" if is_oca else "매도")
            if is_stop_limit:
            ...
            with _str1_sell_state_lock:
                ...
                if is_stop_limit:
                    ...
                    # 동시호가 매도: 09:00 시초가에 체결 예정 → sold=False, oca_ordered=True
                    # 이후 예상가 복귀 시 취소 가능
                    new_st["oca_ordered"] = True
                    new_st["sold"] = False
                else:
                    new_st["sold"] = True
            ...
            oca_suffix = " → 09:00 시초가 체결대기(복귀 시 취소가능)" if is_oca else ""
            msg = (
                ...
                f"  사유={reason}{pnl_txt}{oca_suffix}"
            )
            _notify(msg, tele=True)
            logger.info(msg)
        ...
        if retry_after_cancel:
            ...
            try:
                ...
                with _str1_sell_state_lock:
                    bp = _str1_sell_state.get(code, {}).get("buy_price", 0.0)
                _append_ledger(
                    order_type="SELL_ORDER(재주문)",
                    code=code, name=name,
                    buy_price=float(bp or 0),
                    sell_price=float(ref_price or 0),
                    order_qty=qty,
                    reason=f"{reason}(미체결취소후재주문)",
                    ord_no=ordno2,
                    ord_dvsn=f"{ord_dvsn}({ord_dvsn_name})",
                    prdy_ctrt=float(_last_prdy_ctrt.get(code, 0.0)),
                )
...
def _enqueue_str1_sell(
    ...
    with _str1_sell_state_lock:
        ...
        if is_stop_limit and st.get("stop_limit_ordered"):
            return  # 스톱지정가 중복 방지
        if reason.startswith("str1_시초가하락_예상") and st.get("oca_ordered"):
        ...
        if is_stop_limit:
            ...
            _str1_sell_state[code]["oca_ordered"] = True
        else:
            _str1_sell_state[code]["sell_ordered"] = True
            _str1_sell_state[code]["sold"] = True  # 낙관적 선점
    sell_type = "스톱지정가매도" if is_stop_limit else ("동시호가매도" if is_oca else "매도")
    logger.info(
...
def _enqueue_str1_oca_cancel(code: str, reason: str) -> None:
    """ingest_loop에서 호출: 장전 매도 주문 후 예상가 복귀 시 취소 요청."""
    with _str1_sell_state_lock:
        st = _str1_sell_state.get(code)
        if not st or st.get("sold"):
            return
        if not st.get("oca_ordered"):
            return
        ordno = str(st.get("ordno", "")).strip()
        if not ordno:
            return  # worker가 아직 주문 처리 전
        if st.get("oca_cancel_requested"):
            return  # 중복 취소 방지
        qty = int(st.get("qty") or 0)
        name = st.get("name", code)
        if qty <= 0:
            return
        _str1_sell_state[code]["oca_cancel_requested"] = True
    logger.info(
...
def _ccnl_notice_sub_remove(code: str) -> None:
    """체결 완료 시 H0STCNI0 체결통보 구독 해제. tr_key는 htsid(종목코드 아님)."""
    if code not in _ccnl_notice_sub_codes:
        return
    htsid = _get_ccnl_notice_tr_key()
    if not htsid:
        logger.warning(f"{ts_prefix()} [체결통보] htsid 미설정 → H0STCNI0 해제 불가: {code}")
        return
    with _kws_lock:
        if _active_kws is not None:
            try:
                _send_subscribe(_active_kws, ccnl_notice, [htsid], "2")  # noqa: F405
                _ccnl_notice_sub_codes.discard(code)
                _ccnl_notice_order_ts.pop(code, None)
                name = code_name_map.get(code, code)
                logger.info(f"{ts_prefix()} [체결통보] H0STCNI0 구독 해제: {name}({code}), tr_key={htsid}")
            except Exception as e:
                logger.warning(f"{ts_prefix()} [체결통보] H0STCNI0 해제 실패: {code} {e}")


# ── Str1 실전 매도 상태 ──────────────────────────────────────────────────────
# code → {"sold": bool, "sell_reason": str, "qty": int, "sell_price": float,
#          "sell_time": str, "ordno": str, "pnl": float, "ret_pct": float,
#          "buy_price": float, "oca_ordered": bool}
...
_oca_watch: dict[str, dict] = {}
_oca_last_summary_ts: float = 0.0   # 1분 주기 출력용
OCA_SUMMARY_INTERVAL = 60.0         # 주기적 출력 간격 (초)
_oca_order_summary_done: bool = False  # 08:59:50 주문현황 정리 1회 출력용
_oca_cancelled_log: list[tuple[str, str, str]] = []  # (code, name, reason) 장전 취소 완료
    ...
    for _, row in df.iterrows():
        try:
            ...
            if seln in ("01", "1"):  # 매도
                ...
                with _str1_sell_state_lock:
                    st = _str1_sell_state.get(code)
                    if st and st.get("oca_ordered") and not st.get("sold"):
...
def _prepare_closing_buy_orders() -> None:
    ...
    try:
        accounts = _iter_enabled_accounts()
        if not accounts:
        ...
        _notify(
            ...
            f"({len(accounts)}계좌 × {len(code_prices)}종목, 15:20:00 정시 일괄 발송 대기)",
            tele=True,
        )
    except Exception as e:
...
def _run_closing_buy_orders() -> None:
    ...
    try:
        # 계좌별 클라이언트 캐시 (동일 계좌 반복 생성 방지)
        _acct_clients: dict[str, KisClient] = {}
        accounts = _iter_enabled_accounts()
        for acct in accounts:
            try:
                _acct_clients[acct["cano"]] = _init_account_client(acct)
            except Exception as e:
        ...
        for prep in _closing_buy_prepared:
            code = prep["code"]
            qty = prep["qty"]
            ...
            client = _acct_clients.get(cano, fallback_client)
            try:
                ...
                _append_ledger(
                    ...
                    order_qty=qty,
                    reason=f"종가매수(15:20){acct_tag}",
                    ord_no=ordno,
                    ord_dvsn="01(시장가)",
                    prdy_ctrt=float(prep.get("prdy_ctrt", 0.0)),
                    note=f"상한가={limit_up} 전일종가={prep.get('prev_close', 0)} 계좌={alias}",
                )
            except Exception as e:
...
def _run_overtime_buy_orders() -> None:
    ...
    if not _overtime_order_done and now_t < dtime(16, 1):
        ...
        try:
            ...
            acnt = str(cfg.get("acnt_prdt_cd", "01")).strip() or "01"
            if not cano:
                _notify(f"{ts_prefix()} [시간외단일가] config cano 없음 → 스킵")
                return
            client = _top_client or _init_top_client()
            tr_id = "TTTC0802U"
            for o in unfilled:
                ...
                try:
                    ...
                    _notify(f"{ts_prefix()} [시간외단일가주문] {name}({code}) 수량={remain} 상한가={limit_up:,.0f} (ORD_DVSN=07)")
                except Exception as e:
                    _notify(f"{ts_prefix()} [시간외단일가실패] {name}({code}) {remain}주: {e}")
                    logger.warning(f"{ts_prefix()} [시간외단일가실패] {code}: {e}")
        except Exception as e:
...
def _check_external_orders() -> None:
    ...
    external_orders 각 항목 구조:
      ...
      target_price(0=시장가), ord_type, result, target_qty, remain_qty
...
def _exec_external_order(cfg: dict, order: dict, idx: int) -> None:
    ...
    target_price = float(order.get("target_price", 0) or 0)
    ...
    for acct in target_accounts:
        ...
        try:
            ...
            msg = (f"{ts_prefix()} [외부주문] #{order.get('no',idx+1)} [{alias}] {action} 완료: "
                   f"{name}({code}) qty={qty} {ord_label}({ord_dvsn}) price={price_desc} ordno={ordno}")
            _notify(msg, tele=True)
            logger.info(msg)

            try:
                _append_ledger(
                    order_type=f"external_{action}",
                    code=code, name=name,
                    buy_price=price_val if order_type == 1 else 0,
                    sell_price=price_val if order_type == 2 else 0,
                    order_qty=qty,
                    ord_no=ordno,
                ...
                try:
                    ...
                    if fresh_balance:
                        logger.info(f"{ts_prefix()} [str1_sell] 시작잔고 누락 보충: "
                                    f"{len(fresh_balance)}종목 발견 → 매도 상태 재초기화")
                        _notify(f"{ts_prefix()} [str1_sell] 시작잔고 누락 보충: "
                                f"{len(fresh_balance)}종목 발견", tele=True)
...
def _check_iscd_stat_krx(target_codes: list[str]) -> None:
    """KRX_code_batch로 종목의 iscd_stat_cls_code를 조회하여 _single_price_codes 갱신.
    57=관리종목, 59=단기과열 → 30분 단일가 매매 대상."""
    if not target_codes:
        return
    try:
        ...
        for code, stat in status_map.items():
            if stat in ("57", "59"):
                if code not in _single_price_codes:
                    _single_price_codes[code] = stat
                    name = code_name_map.get(code, code)
                    desc = "57(관리종목)" if stat == "57" else "59(단기과열)"
                    newly_added.append(f"{name}({code})")
                    logger.info(f"[KRX조회] {name}({code}) iscd_stat={stat} ({desc}) → 30분 단일가 매매")
        if newly_added:
            _notify(
                f"{ts_prefix()} [KRX조회] 30분 단일가 종목 {len(newly_added)}건: "
                + ", ".join(newly_added),
                tele=True,
            )
    except Exception as e:
        ...
        for t in target:
            ...
            for a in all_accounts:
                if a["alias"] == t or a["account_id"] == t:
                    if a not in matched:
                        matched.append(a)
        return matched if matched else all_accounts
    return all_accounts
    ...
    with _kws_lock:
        if _active_kws is not None:
            try:
                ...
                name = code_name_map.get(code, code)
                logger.info(f"{ts_prefix()} [VI감지] 예상체결가 구독 추가: {name}({code})")
            except Exception as e:
                logger.warning(f"{ts_prefix()} [VI감지] exp_ccnl 구독 실패: {code} {e}")
    ...
    with _kws_lock:
        if _active_kws is not None:
            try:
                ...
                name = code_name_map.get(code, code)
                logger.info(f"{ts_prefix()} [VI감지] 예상체결가 구독 해제: {name}({code})")
            except Exception as e:
                logger.warning(f"{ts_prefix()} [VI감지] exp_ccnl 해제 실패: {code} {e}")
    ...
    try:
        ...
        with _str1_sell_state_lock:
            _str1_sell_state[code] = {
                ...
                "oca_ordered": False, "balance_qty": qty,
                "source": "vi",
            }
        ...
        logger.info(msg)
        _append_ledger(
            order_type="VI_BUY_ORDER", code=code, name=name,
            buy_price=buy_price, order_qty=qty, reason=reason,
            ord_no=ordno,
        )
    except Exception as e:
...
def _try_vi_buy_cancel(code: str, name: str, antc_prce: float, stck_prpr: float) -> None:
    ...
    try:
        ...
        logger.info(msg)
        _append_ledger(
            order_type="VI_BUY_CANCEL", code=code, name=name,
...
def _on_market_status_krx(result) -> None:
    ...
    for _, row in result.iterrows():
        ...
        if iscd_stat in ("57", "59") and code not in _single_price_codes:
            try:
                krx_map = KRX_code_batch([code])
                krx_stat = krx_map.get(code)
                if krx_stat in ("57", "59"):
                    _single_price_codes[code] = krx_stat
                    desc = "57(관리종목)" if krx_stat == "57" else "59(단기과열)"
                    logger.info(f"{ts_prefix()} [종목상태] {name}({code}) {desc} (H0STMKO0→KRX확인)")
                else:
                    logger.info(
...
def _merge_parts_to_final() -> int:
    ...
    # 스키마 통합: null 타입 컬럼을 utf8로 캐스트 (large_string→null 오류 방지)
...
def _restore_from_snapshot() -> bool:
    ...
    스냅샷이 없거나 당일 것이 아니면 False → 기존 방식(parts 리플레이)으로 폴백.
    """
...
def _check_str1_sell_conditions(
    ...
    is_oca = (kind == "regular_exp")
    is_regular   = (kind == "regular_real")
    if not is_oca and not is_regular:
        return
    ...
    for df_code in df_tmp.partition_by(code_col, maintain_order=True):
        ...
        with _str1_sell_state_lock:
            ...
            if not st or st.get("sold"):
                continue
            # 사전 주문 넣은 상태
            if st.get("oca_ordered"):
                if is_regular:
                    continue  # 09:00 이후, 시초가 체결 예정/체결됨
                # is_oca: 예상가 복귀 시 취소 검사 (아래에서 처리)
    ...
    if (is_oca or is_regular) and _oca_watch:
...
def _print_oca_monitor(newly_triggered: list[str], now_ts: float, is_oca: bool = True) -> None:
    ...
    for code in newly_triggered:
        w = _oca_watch.get(code, {})
        _oca_watch[code]["first_printed"] = True
        prefix = "🔴 [동시호가매도조건 진입]" if is_oca else "🔴 [매도조건 진입]"
        msg = _fmt_oca_alert_line(code, w, prefix=prefix)
        sys.stdout.write(f"\n{msg}\n")
        sys.stdout.flush()
        _notify(msg, tele=True)
        logger.info(msg)
...
def _fmt_oca_alert_line(code: str, w: dict, prefix: str = "") -> str:
...
def _print_oca_order_summary() -> None:
    """08:59:50~09:00 동시호가 주문현황 정리 (체결대기/취소요청/이상없음 구분)."""
    global _oca_order_summary_done
    if _oca_order_summary_done:
        return
    _oca_order_summary_done = True

    cancelled_done: list[str] = []   # 취소 완료 (로그에서)
    with _str1_sell_state_lock:
        cancelled_codes = {c[0] for c in _oca_cancelled_log}
        fill_wait: list[str] = []    # 체결대기 (주문접수완료, 09:00 시초가 체결 예정)
        order_ok: list[str] = []     # 이상없음 (주문 없음)
        cancel_req: list[str] = []   # 취소 요청 중
        for code, name, reason in _oca_cancelled_log:
        ...
        for code, st in _str1_sell_state.items():
            ...
            elif st.get("oca_ordered"):
                fill_wait.append(f"  {name}({code}) 수량={qty}주  주문번호={ordno or '-'}  [주문접수완료→시초가 체결대기]")
            else:
        ...
        if _active_kws is not None:
            try:
                ...
                for req in all_reqs:
                    ...
                    if have:
                        _send_subscribe(_active_kws, req, list(have), "2")
                logger.info("[shutdown] 구독 해제 완료")
            except Exception as e:
                logger.warning(f"[shutdown] 구독 해제 중 에러(무시): {e}")
            _request_ws_close(_active_kws)
    time.sleep(0.8)
...
def _desired_subscription_map(now: datetime) -> dict:
    ...
    if dtime(8, 50) <= t < dtime(9, 0):
        ...
        single_codes = _single_price_codes.keys() & all_codes
