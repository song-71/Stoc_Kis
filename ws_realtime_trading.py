...
# [260424] Strategy A-v4 활성화 (실전 투입 결정: 백테스트 59일 +846%, 승률 38.1%)
UPLIMIT_V4_ENABLED = True           # 5분유지 매수 + 당일홀드 + 29.5% 이상이면 익일MA매도
...
# [260424] Strategy A-v4 상태
_uplimit_v4_28pct_first_ts: dict[str, datetime] = {}   # code → 28% 최초 감지 시각 (이후 28% 이하로 내려가면 reset)
_uplimit_v4_holdover: dict[str, dict] = {}             # code → {buy_price, buy_ts, qty, buy_date, hold_reason, buy_source}
                                                        # 14:55 에 29.5%+ 이었던 종목 (익일까지 보유)
_uplimit_v4_daily_bought_codes: set[str] = set()        # 오늘 이미 v4 매수했던 종목 (중복 방지)
_uplimit_v4_last_sustain_log: dict[str, float] = {}     # sustain 로그 쿨다운 (30초)
...
def _vi_exp_sub_switch(code: str) -> None:
    """VI 발동: a1에서 실시간체결가 해제, a2에서 예상체결가 구독. H0STMKO0은 a2에서 이미 구독 중."""
    ...
    try:
        ...
        logger.warning(f"{ts_prefix()} [VI전환] a2 H0STMKO0 추가 실패 {code}: {e}")


def _vi_exp_sub_restore(code: str) -> None:
    """VI 해제: a2에서 예상체결가 해제, a1에서 실시간체결가 재구독."""
    ...
    with _kws_lock:
        if _active_kws is not None:
            try:
                _send_subscribe(_active_kws, exp_ccnl_krx, [code], "2")  # noqa: F405  a1 fallback 해제 (있으면)
                _send_subscribe(_active_kws, ccnl_krx, [code], "1")      # noqa: F405  실시간 복구
                logger.info(f"{ts_prefix()} [VI복구] {name}({code}) a1 실시간 복구 완료")
            except Exception as e:
                logger.warning(f"{ts_prefix()} [VI복구] {name}({code}) a1 구독 복구 실패: {e}")
    # ── H0STMKO0 keep-alive 순환: a2에서 처리 ──
    ...
    if market:
        ...
        if prev_keepalive != code:
            ...
            if (prev_keepalive
                ...
                try:
                    _a2_mkstatus_sub_remove({prev_keepalive})
                    logger.info(f"{ts_prefix()} [VI복구] a2 keep-alive 순환: {prev_keepalive}→{code} ({market})")
                except Exception as e:
                    logger.warning(f"{ts_prefix()} [VI복구] a2 이전 keep-alive 해제 실패 {prev_keepalive}: {e}")
...
def _mkstatus_sub_add(codes_to_add: set[str]) -> None:
    """보유종목에 대해 H0STMKO0 장운영정보 구독 추가 → a2 WSS로 위임."""
    _a2_mkstatus_sub_add(codes_to_add)


def _mkstatus_sub_remove(codes_to_remove: set[str]) -> None:
    """매도 완료 종목의 H0STMKO0 구독 해제 → a2 WSS로 위임."""
    _a2_mkstatus_sub_remove(codes_to_remove)
...
def _handle_stale_check_result(code: str, output: dict) -> None:
    ...
    **전혀 다른 의미**이므로 종목상태(상장폐지/관리종목 등) 판단에 사용하면 안 됨.
    ...
    if temp_stop == "Y":
        ...
        # VI 발동 여부 REST 확인 (이미 예상체결 구독 중이면 스킵)
        if code not in _vi_exp_sub_ts:
            try:
                ...
                if is_vi:
                    ...
                    vi_label = _VI_CLS_NAMES.get(vi_cls, vi_cls)
                    msg = (f"{ts_prefix()} [VI감지-REST] {name}({code}) "
                           f"{vi_label} 발동시각={vi_time} → 예상체결 전환")
                    logger.warning(msg)
                    _notify(msg, tele=True)
                    _vi_exp_sub_switch(code)
            except Exception as e:
                ...
                if available_slots < len(candidates):
                    ...
                    if evicted_codes:
                        ...
                        if _removed_lru:
                            ...
                            _notify(msg_lru)
                            changed = True
                        available_slots = max(0, MAX_WSS_SUBSCRIBE - len(codes))
...
# =============================================================================
# [260424] Strategy A-v4 — 28% 5분유지 + 당일홀드 + 상한가 익일MA매도
# =============================================================================
def _check_uplimit_v4_from_tick(
    ...
    """v4 전략: 매 틱 호출.
    - 28% 최초 감지 시각 기록 (_uplimit_v4_28pct_first_ts)
    - 28% 미만으로 내려가면 감지 시각 리셋
    - 5분 유지 확인 후 매수 시도
    - 보유 종목: -5% 하드손절, 14:55 에 29.5%+ 면 overnight 전환, 아니면 마감 청산
    - overnight 종목: 익일 _check_v4_nextday_exit 로직이 담당
    """
    if kind != "regular_real":
        return
    ...
    for df_code in df_tmp.partition_by(code_col, maintain_order=True):
        try:
            ...
            # ── 28% 최초 감지 / 리셋 ─────────────────────────────────────
            if prdy_ctrt >= 28.0 and prdy_ctrt < 30.0:
                if code not in _uplimit_v4_28pct_first_ts:
                    _uplimit_v4_28pct_first_ts[code] = now_dt
            elif prdy_ctrt < 28.0:
                _uplimit_v4_28pct_first_ts.pop(code, None)

            # ── 보유 포지션 (당일) 관리 ───────────────────────────────────
            if code in _uplimit_positions and _uplimit_positions[code].get("source") == "v4":
                pos = _uplimit_positions[code]
                if bidp1 > pos.get("highest_since_buy", 0):
                    pos["highest_since_buy"] = bidp1
                if prdy_ctrt > pos.get("max_prdy_ctrt", 0):
                ...
                exit_reason = None
                # (a) 하드손절 -5%
                if low > 0 and low <= pos["buy_price"] * (1 - UPLIMIT_V4_HARD_LOSS):
                    exit_reason = f"v4_하드손절-{int(UPLIMIT_V4_HARD_LOSS*100)}%"
                # (b) 14:55 판정
                elif (h, m) >= (14, 55):
                    ...
                    if should_hold:
                        ...
                        exit_reason = hr or f"v4_당일청산({prdy_ctrt:.2f}%)"

                if exit_reason:
                    try:
                        _try_v4_market_sell(code, reason=exit_reason)
                    except Exception as _ex:
                        logger.warning(f"{ts_prefix()} [v4_exit] {code} 매도 실패: {_ex}")
                continue  # 보유 종목은 여기서 처리 완료

            # ── 매수 판정 (5분 유지 후) ──────────────────────────────────
            # overnight 이거나 이미 당일 매수했던 종목은 skip
            ...
            first_ts = _uplimit_v4_28pct_first_ts.get(code)
            sustain_min = None
            if first_ts is not None:
                sustain_min = (now_dt - first_ts).total_seconds() / 60.0

            # 활성 포지션 수 체크 (분산)
            active_n = sum(
                1 for c, p in _uplimit_positions.items()
                if p.get("source") == "v4" and not p.get("sold", False)
            ) + len(_uplimit_buy_pending)
            if active_n >= UPLIMIT_DIVERSIFY_N:
                continue

            should_buy, reason = check_uplimit_v4_sustain_buy(
                prdy_ctrt=prdy_ctrt,
                first_28_seen_minutes_ago=sustain_min,
                already_holding=False,
                now_hm=(h, m),
            )
            if not should_buy:
                # sustain 로그 (종목당 30초 쿨다운)
                _last = _uplimit_v4_last_sustain_log.get(code, 0.0)
                if sustain_min is not None and sustain_min >= 3 and time.time() - _last > 30:
                    _uplimit_v4_last_sustain_log[code] = time.time()
                    logger.info(
                        f"{ts_prefix()} [v4_waiting] {code_name_map.get(code, code)}({code}) "
                        f"sustain={sustain_min:.1f}분, ctrt={prdy_ctrt:.2f}% — {reason}"
                    )
                continue

            # 매수 실행
            try:
                _try_v4_buy(code, prdy_ctrt, stck_prpr, now_dt, reason)
            except Exception as _eb:
                logger.warning(f"{ts_prefix()} [v4_buy] {code} 실패: {_eb}")
        except Exception as _e:
            logger.debug(f"[v4_tick] 예외 {code if 'code' in locals() else '?'}: {_e}")


def _try_v4_buy(code: str, prdy_ctrt: float, stck_prpr: float, now_dt, reason: str):
    """v4 매수 주문. source='v4' 로 _uplimit_positions 에 등록."""
    name = code_name_map.get(code, code)
    accts = _iter_enabled_accounts(trade_only=True)
    if not accts:
        return
    ...
    if avail_cash <= 0:
        logger.info(f"{ts_prefix()} [v4_buy] {name}({code}) 잔고 0 → 스킵")
        return
    # 시드 베이스 분할
    seed_base = _uplimit_seed_base if _uplimit_seed_base > 0 else (avail_cash / UPLIMIT_DIVERSIFY_N)
    invest = min(seed_base, avail_cash)
    entry_price = calc_entry_price(stck_prpr, "KOSPI", UPLIMIT_ENTRY_TICKS)
    qty = calc_uplimit_qty(invest, entry_price, "KOSPI")
    if qty <= 0:
        logger.info(
            f"{ts_prefix()} [v4_buy] {name}({code}) 수량=0 "
            f"(seed={int(seed_base):,}원, 단가={int(entry_price):,}원)"
        )
        return

    j = _buy_order_cash(
        client, acct["cano"], acct.get("acnt_prdt_cd", "01") or "01",
        "TTTC0802U", code, qty, entry_price, ord_dvsn="00"
    )
    out = j.get("output") or {}
    ordno = str(out.get("ODNO") or out.get("odno") or "").strip()
    ...
    msg = (
        f"{ts_prefix()} [v4_매수] {name}({code}) ctrt={prdy_ctrt:.2f}%  "
        f"수량={qty}  단가={int(entry_price):,}  ord={ordno}  사유={reason}"
    )
    _notify(msg, tele=True)
    logger.info(msg)
    _append_ledger(
        order_type="buy_order", code=code, name=name,
        buy_price=entry_price, order_qty=qty, reason=f"v4_{reason}",
        ord_no=ordno, stck_prpr=stck_prpr,
    )
...
def _shutdown(reason: str):
    ...
    with _a2_kws_lock:
        if _a2_active_kws is not None:
            try:
                _request_ws_close(_a2_active_kws)
            except Exception:
                pass
            _a2_active_kws = None
    t_wss = time.time()
...
def _get_a2_approval_key() -> str:
    ...
    resp = requests.post(url, data=json.dumps(p), headers=headers, timeout=10)
    if resp.status_code != 200:
        raise RuntimeError(f"[a2-WSS] approval_key 발급 실패: HTTP {resp.status_code} {resp.text[:200]}")
    key = resp.json().get("approval_key", "")
    if not key:
        raise RuntimeError(f"[a2-WSS] approval_key 비어있음: {resp.text[:200]}")
    return key


def _on_result_a2(ws, tr_id, result, data_info):
    ...
    if trid == "H0STMKO0":
        _on_market_status_krx(result)
        return
    # H0STANC0 (예상체결가) — VI 종목 예상체결 데이터
    if trid in KNOWN_TRID_MAP or trid in TRID_TO_KIND:
        kind = TRID_TO_KIND.get(trid)
        is_real = TRID_TO_IS_REAL.get(trid)
        if kind is None and trid in KNOWN_TRID_MAP:
            kind, is_real = KNOWN_TRID_MAP[trid]
            TRID_TO_KIND[trid] = kind
            TRID_TO_IS_REAL[trid] = is_real
        recv_ts = datetime.now(KST).strftime("%Y-%m-%d %H:%M:%S.%f")
...
def run_ws_a2_forever():
    ...
    # approval_key 발급
    try:
        _a2_approval_key = _get_a2_approval_key()
        logger.info(f"{ts_prefix()} [a2-WSS] approval_key 발급 완료")
    except Exception as e:
        logger.error(f"{ts_prefix()} [a2-WSS] approval_key 발급 실패: {e} — a2 WSS 비활성")
    ...
    while not _stop_event.is_set():
        ...
        kws = None
        try:
            logger.info(f"{ts_prefix()} [a2-WSS] connect attempt={attempt}")

            # 재연결 시 approval_key 갱신
            if attempt > 1:
                try:
                    _a2_approval_key = _get_a2_approval_key()
                    logger.info(f"{ts_prefix()} [a2-WSS] approval_key 재발급 완료")
                except Exception as ae:
                    logger.warning(f"{ts_prefix()} [a2-WSS] approval_key 재발급 실패: {ae}")

            # a2는 open_map 사용하지 않음 (독립 인스턴스)
            kws = ka.KISWebSocket(api_url="", approval_key=_a2_approval_key)
            with _a2_kws_lock:
                _a2_active_kws = kws
            ...
            # 연결 후 콜백에서 초기 구독 실행
            # kws.start()는 블로킹이므로 먼저 구독 세팅 후 start
            # 그러나 send_request는 start 이후에만 가능 → start 직후 별도 스레드에서 구독
            def _a2_initial_subscribe():
                """a2 연결 직후 H0STMKO0 전 종목 구독."""
                time.sleep(1.0)  # WebSocket 연결 안정화 대기
                try:
                    # 구독 대상: 보유종목 + keepalive 종목 + codes (전 종목)
                    held_codes = set()
                    try:
                        with _str1_sell_state_lock:
                            held_codes = {c for c, st in _str1_sell_state.items() if not st.get("sold")}
                        ...
                        pass
                    all_mko = set(codes) | held_codes
                    for mkt, rep_code in _MKT_KEEPALIVE_INITIAL.items():
                        all_mko.add(rep_code)
                    for c in _mkt_keepalive_current.values():
                        if c:
                            all_mko.add(c)

                    if all_mko:
                        mko_list = list(all_mko)
                        with _a2_kws_lock:
                            if _a2_active_kws is not None:
                                for i in range(0, len(mko_list), MAX_CODES_PER_SESSION):
                                    batch = mko_list[i:i + MAX_CODES_PER_SESSION]
                                    try:
                                        _a2_active_kws.send_request(request=market_status_krx, tr_type="1", data=batch)  # noqa: F405
                                    except Exception as e:
                                        logger.warning(f"{ts_prefix()} [a2-WSS] H0STMKO0 구독 실패 (batch): {e}")
                                _a2_subscribed["market_status_krx"] = set(mko_list)
                                _mkstatus_sub_codes.update(all_mko)
                        logger.info(
                            f"{ts_prefix()} [a2-WSS] H0STMKO0 구독 완료: {len(all_mko)}종목"
                        )
                    # VI 활성 종목이 있으면 예상체결가도 구독
                    vi_codes = list(_vi_active_codes)
                    if vi_codes:
                        with _a2_kws_lock:
                            if _a2_active_kws is not None:
                                for i in range(0, len(vi_codes), MAX_CODES_PER_SESSION):
                                    batch = vi_codes[i:i + MAX_CODES_PER_SESSION]
                                    try:
                                        _a2_active_kws.send_request(request=exp_ccnl_krx, tr_type="1", data=batch)  # noqa: F405
                                    except Exception as e:
                                        logger.warning(f"{ts_prefix()} [a2-WSS] 예상체결 구독 실패: {e}")
                                _a2_subscribed["exp_ccnl_krx"] = set(vi_codes)
                        logger.info(f"{ts_prefix()} [a2-WSS] VI 예상체결 구독: {len(vi_codes)}종목")
                except Exception as e:
                    logger.error(f"{ts_prefix()} [a2-WSS] 초기 구독 실패: {e}")

            t_sub = threading.Thread(target=_a2_initial_subscribe, daemon=True)
            t_sub.start()

            logger.info(f"{ts_prefix()} [a2-WSS] start on_result")
            kws.start(on_result=_on_result_a2)
            logger.info(f"{ts_prefix()} [a2-WSS] kws.start returned")

        except SystemExit:
            break
        except Exception as e:
            logger.error(f"{ts_prefix()} [a2-WSS] connection exception: {e}")
            logger.error(traceback.format_exc())
        finally:
            with _a2_kws_lock:
                if _a2_active_kws is kws:
                    _a2_active_kws = None
            _a2_subscribed.clear()
...
def run_ws_forever():
    ...
    while not _stop_event.is_set():
        ...
        try:
            ...
            # H0STMKO0 → a2 WSS로 이관됨 (a1에서 제거)
            # ── 슬롯 상한 준수: H0STCNI0(1) + 종목데이터(M) <= 40 ──
            ...
            total_sub = ccnl_notice_slots + data_slots

            if total_sub > MAX_WSS_SUBSCRIBE:
                max_data = MAX_WSS_SUBSCRIBE - ccnl_notice_slots
                logger.warning(
...
if __name__ == "__main__":
    ...
    # a2 (syw_2) 듀얼 WSS: H0STMKO0 장운영정보 + VI 예상체결가 전담
    t_a2_wss = threading.Thread(target=run_ws_a2_forever, daemon=True, name="a2-WSS")
    t_a2_wss.start()
    logger.info(f"{ts_prefix()} [a2-WSS] 데몬 스레드 시작")
