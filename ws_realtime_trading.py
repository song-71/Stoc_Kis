...
STR2_ENABLED = False
# [260722] str2 시뮬레이션(장부) 모드 — True 면 str2 전략을 '실거래 없이' 구동해
#   트리거·주문·체결·청산을 전부 _append_ledger 장부에 기록만 한다(실주문·취소 미발주).
#   config["str2"]["simul_mode"]==true 로 켜지며(_refresh_str2_enabled 갱신). 라이브(UPLIMIT_V5)에 무영향.
#   sim 체결은 '주문 시점 즉시 체결'(지정가=주문가, 시장가=현재가)로 간주 — 실제 체결 정밀도(터치·부분체결)는
#   기록된 wss_data 틱으로 오프라인 검증(compare_orderbook_fill 등)한다.
STR2_SIMUL_MODE = True   # [260722] 모의투자(장부) 기본 ON — 실거래는 STR2_ENABLED(별도)만 True 일 때. config 로 override 가능.
# [260722] 상한가 눌림목 검증본 정합 — True 면 '장중 눌림목 매수(b3)'만 작동시키고
#   장전하락(b1)·시초시장가(b2) 매수 경로를 비활성화한다. 매도는 상한가 재도달/ma500<ma2000 데드/EOD.
#   손절·트레일은 sell_stop_trail 이 항상 미발동(검증본: 손절이 오히려 손해).
STR2_PULLBACK_ONLY = True
...
STR2_MAXE = 0                     # 종목당 하루 최대 진입 횟수 (0=무제한). [260722] 상한가 눌림목만 진입하므로 제한 해제
STR2_SHALLOW = 0.02              # 얕은 눌림 경계(2%): 이하=지정가 전용
STR2_SIZE_LIM_MAX = 40_000_000  # 깊은 눌림(>2%) 시 투입액 이 값 이하=지정가 / 초과=시장가 스윕
STR2_MKT_CLOSE_SEC = 15 * 3600 + 30 * 60   # 정규장 마감 15:30 — 이후 오더북 구독 중단·해제(시간외 캡처 방지)
...
_str2_ma10_buf: dict[str, deque] = {}        # code → 직전 50틱 ma10 deque (검증본: 지정가=ma10 최저)

# [260722] 눌림목 트리거 오더북(H0STASP0) 메인 통합 — 별도 프로세스·공유메모리 버스 불필요.
#   상한가 이탈(미보유) 시 그 종목만 오더북 구독 → 매도 청산 시 해제.
#   신호 없이 상한가 미복귀 시 최대 STR2_OB_HOLD_SEC 초만 유지(자동 해제).
#   구독/해제 send_request 는 tick 파이프라인과 분리된 전용 워커에서 수행(이벤트루프 자기교착 방지).
STR2_OB_ON_TRIGGER = True                     # 눌림목 트리거 오더북 구독 활성(str2 SIMUL/실거래 동작 시에만 작동)
STR2_OB_HOLD_SEC = 300                         # 이탈 후 신호없이 유지 최대 초(5분)
_str2_ob_latest: dict[str, dict] = {}          # code → 최신 오더북(askp/bidp/rsqn 10단계) — _get_orderbook 소비
_str2_ob_subs: dict[str, float] = {}           # code → 구독 시작 epoch (5분 타임아웃/청산 해제 관리)
...
def _refresh_str2_enabled() -> None:
    ...
    """
    global STR2_ENABLED, STR2_SIMUL_MODE
    try:
        cfg = _read_cfg()
        if not cfg:
            return
        ...
        if enabled != STR2_ENABLED:
            ...
            _notify(msg, tele=True)
            logger.info(msg)
        # [260722] 시뮬레이션(장부) 모드 갱신 — config["str2"]["simul_mode"] 키가 있으면 그 값,
        #   없으면 모듈 기본(STR2_SIMUL_MODE=True) 유지 → config 미설정 시에도 모의투자 동작.
        if isinstance(s2, dict) and ("simul_mode" in s2):
            simul = bool(s2.get("simul_mode") is True)
        else:
            simul = STR2_SIMUL_MODE
        if simul != STR2_SIMUL_MODE:
            STR2_SIMUL_MODE = simul
            msg = f"{ts_prefix()} [str2] 시뮬레이션(장부) 모드 {'ON(실거래 없음)' if simul else 'OFF'} (config 반영)"
            _notify(msg, tele=True)
            logger.info(msg)
    except Exception as e:
...
def _save_str2_state(force: bool = False) -> None:
    ...
    if not (STR2_ENABLED or STR2_SIMUL_MODE):   # [260722] SIMUL 도 상태 저장(재시작 연속성)
        return
    now_ts = time.time()
...
def _restore_str2_state_on_startup() -> None:
    ...
    if not (STR2_ENABLED or STR2_SIMUL_MODE):   # [260722] SIMUL 도 복원
        return
    try:
        path = _str2_state_path()
        if not path.exists():
...
REQ_STREAM_INFO: dict[str, tuple[str, str]] = {
    ...
    "asking_price_krx":      ("H0STASP0", "호가(오더북)"),
}
...
def _str2_simul_ob_fill(code: str, use_market: bool, ord_price: float, qty: int,
    ...
    if side == "buy":
        ...
        if use_market:
            rem = qty; sh = 0; spent = 0.0
            for px, q in zip(px_lv, rsqn_lv):
                if not px or not q: continue
                take = min(rem, int(q)); sh += take; spent += take * px; rem -= take
                if rem <= 0: break
            if sh <= 0:
                return 0, 0.0, "매도호가없음→미체결"
            vwap = spent / sh
        ...
        px_lv = ob.get("bidp") or []; rsqn_lv = ob.get("bidp_rsqn") or []
        if use_market:
            rem = qty; sh = 0; spent = 0.0
            for px, q in zip(px_lv, rsqn_lv):
                if not px or not q: continue
                take = min(rem, int(q)); sh += take; spent += take * px; rem -= take
                if rem <= 0: break
            if sh <= 0: return 0, 0.0, "매수호가없음→미체결"
            return sh, spent / sh, f"시장가 매도 호가스윕 {sh}/{qty}주"
        else:
...
def _str2_place_buy(code: str, st: "Str2State", price: float, stck_prpr: float,
    ...
    if not use_market and ord_price <= 0:
        ...
        return
    if STR2_SIMUL_MODE:
        ordno = "SIMUL"                       # [260722] 시뮬레이션: 실주문 미발주(장부만)
    else:
        try:
            j = _buy_order_cash(
                client, acct["cano"], acct.get("acnt_prdt_cd", "01") or "01",
    ...
    _pfx = "SIMUL_" if STR2_SIMUL_MODE else ""
    dvsn_label = "시장가" if use_market else f"지정가({int(ord_price):,})"
    _obs = f" | OB[{ob_note}]" if STR2_SIMUL_MODE else ""
    msg = (f"{ts_prefix()} [str2_매수{'(SIMUL)' if STR2_SIMUL_MODE else ''}] {name}({code}) {origin} {dvsn_label} qty={qty} "
           f"현재가={int(stck_prpr):,} ord={ordno} 사유={reason}{_obs}")
    _notify(msg, tele=not STR2_SIMUL_MODE)
    logger.info(msg)
    _append_ledger(order_type="buy_order", code=code, name=name, buy_price=stck_prpr,
                   order_qty=qty, reason=f"str2_{_pfx}{reason}", ord_no=ordno,
                   ord_dvsn=ord_dvsn, stck_prpr=stck_prpr, note=(ob_note if STR2_SIMUL_MODE else ""))
    if STR2_SIMUL_MODE and fill_qty > 0:   # orderbook 판정 즉시체결 장부 기록
        _append_ledger(order_type="buy_fill", code=code, name=name, buy_price=fill_pr,
                       fill_qty=fill_qty, reason="str2_SIMUL_매수체결",
                       note=f"orderbook판정: {ob_note}", stck_prpr=stck_prpr)
...
def _str2_place_sell(code: str, st: "Str2State", qty: int, price: float,
    ...
    ob_note = ""; fill_qty = 0; fill_pr = 0.0
    if STR2_SIMUL_MODE:
        ordno = "SIMUL"                        # [260722] 시뮬레이션: 실주문 미발주
        fill_qty, fill_pr, ob_note = _str2_simul_ob_fill(code, (ord_dvsn == "01"), float(ord_price), qty, stck_prpr, "sell")
    else:
        try:
            j = _sell_order_cash(client, acct["cano"], acct.get("acnt_prdt_cd", "01") or "01",
    ...
    with _str2_state_lock:
        if STR2_SIMUL_MODE:
            if fill_qty > 0:
                st.qty = max(0, st.qty - fill_qty)
                if st.qty <= 0:
                    st.position = False; st.sell_order_active = False; st.sell_ready = False
            else:
                st.sell_order_active = (ord_dvsn == "00")   # 지정가 대기
                if ord_dvsn == "00": st.sell_order_price = float(ord_price)
        elif ord_dvsn == "00":
    ...
    _obs = f" | OB[{ob_note}]" if STR2_SIMUL_MODE else ""
    msg = (f"{ts_prefix()} [str2_매도{'(SIMUL)' if STR2_SIMUL_MODE else ''}] {name}({code}) {dvsn_label} qty={qty} "
           f"현재가={int(stck_prpr):,} ord={ordno} 사유={reason}{_obs}")
    _notify(msg, tele=not STR2_SIMUL_MODE)
    logger.info(msg)
    _append_ledger(order_type="sell_order", code=code, name=name,
                   buy_price=st.buy_price, sell_price=stck_prpr, order_qty=qty,
                   reason=f"str2_{_pfx}{reason}", ord_no=ordno, ord_dvsn=ord_dvsn,
                   stck_prpr=stck_prpr, note=(ob_note if STR2_SIMUL_MODE else ""))
    if STR2_SIMUL_MODE and fill_qty > 0:
        _append_ledger(order_type="sell_fill", code=code, name=name, buy_price=st.buy_price,
                       sell_price=fill_pr, fill_qty=fill_qty, reason="str2_SIMUL_매도체결",
                       note=f"orderbook판정: {ob_note}", stck_prpr=stck_prpr)
...
def _str2_cancel(code: str, st: "Str2State", ordno: str, qty: int, ord_dvsn: str, reason: str) -> None:
    ...
    if not accts:
        return
    if STR2_SIMUL_MODE:                        # [260722] 시뮬레이션: 실제 취소 미발주(로그·장부만)
        msg = f"{ts_prefix()} [str2_취소(SIMUL)] {name}({code}) ord={ordno} 사유={reason}"
        _notify(msg, tele=False); logger.info(msg)
        _append_ledger(order_type="buy_cancel", code=code, name=name, order_qty=qty,
                       reason=f"str2_SIMUL_{reason}", ord_no=ordno, ord_dvsn=ord_dvsn)
        return
    acct = accts[0]
    client = _init_account_client(acct)
    try:
...
def _check_str2_from_tick(result: "pl.DataFrame", col_map: dict, code_col: str, kind: str) -> None:
    ...
    if not (STR2_ENABLED or STR2_SIMUL_MODE):   # [260722] sim 모드도 구동(실주문은 _str2_place_* 에서 게이트)
        return
    if kind != "regular_real":
        return
        ...
        try:
            ...
            if STR2_SIMUL_MODE and (st.buy_order_active or st.sell_order_active):
                ...
                if st.sell_order_active and st.qty > 0:
                    ...
                    if _fq > 0:
                        st.qty = max(0, st.qty - _fq)
                        if st.qty <= 0:
                            st.position = False; st.sell_order_active = False; st.sell_ready = False
                            _str2_ob_unsubscribe(code, "SIMUL청산")   # [260722] 청산 → 오더북 해제
                        _append_ledger(order_type="sell_fill", code=code, name=nm2, buy_price=st.buy_price,
                                       sell_price=_fp, fill_qty=_fq, reason="str2_SIMUL_지정가매도체결",
                                       note=f"orderbook판정: {_obn}", stck_prpr=stck_prpr)
                        logger.info(f"{ts_prefix()} [str2_체결(SIMUL)] 매도 {nm2}({code}) {_fq}주×{int(_fp):,} | {_obn}")
            ...
            # [260722] 검증본: 지정가 매수가 = 직전 50틱 ma10 최저 (구 bidp1 최저 대체)
            m10buf = _str2_ma10_buf.get(code)
            if m10buf is None:
                m10buf = deque(maxlen=STR2_ROLL); _str2_ma10_buf[code] = m10buf
            if ma10 > 0:
                m10buf.append(ma10)
            roll_min_ma10 = min(m10buf) if m10buf else 0.0

            ma500_up, ma2000_up = strat2.compute_trends(
                ma50, ma200, ma300, ma500, ma2000, wghn, tick_idx)

            # [260722] 검증본: 정배열(ma500>=ma2000) 관측 래치 → 이후 하향교차 시 매도(sell_deadcross)
            ...
            if code not in _str2_session_setup_done:
                ...
                if now_sec >= strat2.T_0858:
                    ...
                    market = _code_market_map.get(code, "KOSPI") or "KOSPI"
                    # [260722] 검증본: 눌림목(b3)만 → 장전하락(b1)·시초시장가(b2) 비활성
                    st.b1_enabled = b1 and not STR2_PULLBACK_ONLY
                    st.b2_pending = b2 and not STR2_PULLBACK_ONLY
            ...
            if st.limit_up > 0:
                if strat2.is_limit_up(stck_prpr, prdy_ctrt, st.limit_up):
                    ...
                    if STR2_OB_ON_TRIGGER and not st.position and now_sec < STR2_MKT_CLOSE_SEC:
                        _str2_ob_subscribe(code)
                if STR2_OB_ON_TRIGGER and st.position and code not in _str2_ob_subs and now_sec < STR2_MKT_CLOSE_SEC:
                    _str2_ob_subscribe(code)
                # [260722] 정규장 마감(15:30) 후 오더북 구독 해제 — 시간외 호가 캡처 방지(point 3)
                if now_sec >= STR2_MKT_CLOSE_SEC and code in _str2_ob_subs:
                    _str2_ob_unsubscribe(code, "장마감")
            ...
            if st.position:
                ...
                # (a0) [260722] 검증본: 상한가 재도달 → 즉시 지정가 매도(상한가는 매수대기 풍부→체결 용이)
                if st.limit_up > 0 and strat2.is_limit_up(stck_prpr, prdy_ctrt, st.limit_up):
                    if not st.sell_order_active:
                        st.sell_order_tick = tick_idx
                        _str2_place_sell(code, st, st.qty, price=st.limit_up, use_limit=True,
                                         reason="str2_상한가재도달_지정가", stck_prpr=stck_prpr)
                    st.ma10_prev, st.ma500_prev, st.ma2000_prev = ma10, ma500, ma2000
                    continue

                # [260722] 검증본(sweep_exec_sim) 청산은 3가지뿐 — 상한가재도달(위 a0)/ma500<ma2000 데드(아래 e)/EOD(f).
                #   구 str2 의 손절·트레일·sell_ready·ma500gap·지정가매도(구 b/c/d)는 검증본에 없어 제거.
                #   (a0 상한가 지정가 매도 체결은 상단 SIMUL orderbook 재점검이 처리.)

                # (e) [260722] 검증본: ma500<ma2000 데드크로스(골든 관측 후) — ma2000 워밍업 전엔 폴백(ma10<ma500)
                do3, r3 = strat2.sell_deadcross(False, st.ma10_prev, st.ma500_prev, ma10, ma500,
                                                st.ma2000_prev, ma2000, st.golden_seen)
                if do3:
    ...
    _str2_ob_timeout_sweep()   # [260722] 이탈 후 무신호 5분 경과 종목 오더북 자동 해제
...
def _check_str2_carry_premarket(result: "pl.DataFrame", col_map: dict, code_col: str, kind: str) -> None:
    ...
    """
    if not (STR2_ENABLED or STR2_SIMUL_MODE):   # [260722] SIMUL 도 carry_hold 익일 매도
        return
    if kind != "regular_exp":
        return
...
def ingest_loop():
    ...
    while not _stop_event.is_set() or not _ingest_queue.empty():
        ...
        # ── [260523] Strategy str2 매수/매도 (STR2_ENABLED 실거래 / STR2_SIMUL_MODE 모의투자) ──
        if STR2_ENABLED or STR2_SIMUL_MODE:
            try:
...
def _shutdown(reason: str):
    ...
    try:
        ...
        t_flush = time.time()
    try:
        with _str2_ob_save_lock:   # [260722] 오더북 잔여 버퍼 flush + parts 병합
            _flush_str2_ob_save()
        _merge_str2_ob_parts()
    except Exception as e:
        logger.warning(f"[shutdown] 오더북 parts 병합 실패: {e}")
    try:
        _save_indicator_snapshot()
        t_snap = time.time()
...
def _build_sub_map(now: datetime, code_subset) -> dict:
    """[260715] 주어진 종목 부분집합에 대한 구독 매핑 {request_func: set(종목코드)}.
    기존 _desired_subscription_map 본문을 그대로 추출하되 코드 소스만 subset 으로 교체
    (다계좌 a1/a2 분할용). VI/57·59/서킷 종목만 예상체결, 나머지 실시간체결."""
    ...
    if dtime(15, 30) <= t < dtime(16, 0):
        ...
        if len(_regular_real_seen) >= len(all_codes):
            return {}
...
# ── [260715] 다계좌 구독 총괄관리자 (S4) — a1/a2 배정 + 당일 28%↑ sticky ──
# a1 데이터 예산 = MAX_WSS_SUBSCRIBE - 1(H0STCNI0). (H0STMKO0 는 a2 로 이관: S3)
#   플래그 OFF 동안엔 어떤 것도 관여하지 않는다(= 기존 단일 a1 동작).
def _str2_ob_count(acct: str) -> int:
    """현재 해당 계좌(a1/a2)에 걸린 눌림목 오더북(H0STASP0) 구독 수."""
    try:
        return sum(1 for a in _str2_ob_acct.values() if a == acct)
    except Exception:
        return 0


def _a1_data_budget() -> int:
    # [260722] a1 오더북(H0STASP0) 구독분만큼 데이터 예산 축소 → 총 40 슬롯 초과 방지
    return max(1, MAX_WSS_SUBSCRIBE - 1 - _str2_ob_count("a1"))


def _a2_data_budget() -> int:
    """a2 연결의 종목데이터 예산 = 40 - H0STMKO0(장운영정보) - a2 오더북 슬롯.
...
def _partition_a1_a2():
    ...
    try:
        ...
        pass
    held &= universe
    vi = (_vi_active_codes & universe) - held
...
def run_ws_forever():
    ...
    while not _stop_event.is_set():
        ...
        try:
            ...
            ka.open_map.clear()
            _subscribed.clear()
            _str2_ob_reset("a1")   # [260722] a1 재접속 → 오더북 재구독 대기(트리거/보유가드가 재구독)
...
def _get_orderbook(code: str):
    """전략용: 종목 최신 호가 조회. 우선순위 ① 메인통합 스토어(_str2_ob_latest) ② 공유메모리 버스.
    반환: {code,name,recv_epoch,bsop_hour,askp[10],bidp[10],askp_rsqn[10],bidp_rsqn[10],...} 또는 None."""
    c = str(code).zfill(6)
    ob = _str2_ob_latest.get(c)          # [260722] 눌림목 트리거 메인 통합 우선
    if ob is not None:
        return ob
    b = _orderbook_bus
    if b is None:
...
def _rec_i(rec: dict, key: str) -> int:
    """오더북 필드 → int (콤마·공백·빈값 방어)."""
    try:
        return int(float(str(rec.get(key, 0) or 0).replace(",", "").strip() or 0))
    except (ValueError, TypeError):
        return 0
...
def _flush_str2_ob_save() -> None:
    """오더북 저장 버퍼 → parquet part(zstd). 호출자가 _str2_ob_save_lock 보유. (CSV 대비 용량·속도 우수)"""
    global _str2_ob_save_last, _str2_ob_part_seq
    if not _str2_ob_save_buf:
        return
    try:
        d = OUT_DIR / "orderbook" / "parts"
        d.mkdir(parents=True, exist_ok=True)
        _str2_ob_part_seq += 1
        path = d / f"{today_yymmdd}_orderbook_{_str2_ob_part_seq:04d}.parquet"
        n = len(_str2_ob_save_buf)
        pl.DataFrame(_str2_ob_save_buf).select(_STR2_OB_SAVE_COLS).write_parquet(path, compression="zstd")
        _str2_ob_save_buf.clear()
        _str2_ob_save_last = time.time()
        logger.info(f"{ts_prefix()} [str2_ob] 오더북 {n}건 저장 → parts/{path.name}")
    except Exception as e:
        logger.warning(f"{ts_prefix()} [str2_ob] 오더북 저장 실패: {type(e).__name__}: {e}")


def _merge_str2_ob_parts() -> None:
    """오더북 parts/*.parquet → 일별 단일 parquet 병합(종료 시 1회)."""
    try:
        pdir = OUT_DIR / "orderbook" / "parts"
        parts = sorted(pdir.glob(f"{today_yymmdd}_orderbook_*.parquet"))
        if not parts:
            return
        df = pl.concat([pl.read_parquet(p) for p in parts])
        out = OUT_DIR / "orderbook" / f"{today_yymmdd}_orderbook.parquet"
        df.write_parquet(out, compression="zstd")
        logger.info(f"{ts_prefix()} [str2_ob] 오더북 parts {len(parts)}개 병합 → {out.name} ({len(df):,}행)")
    except Exception as e:
...
def _str2_ob_ingest(result: "pl.DataFrame") -> None:
    ...
    for rec in recs:
        ...
        # [260722] 쓰레기행 제거: H0STASP0 프레임마다 딸려오는 '가격을 code로 가진 all-zero 행' 필터
        #   (실측 47% 차지). 유효 호가(askp1>0 or bidp1>0) 없으면 latest·저장 모두 스킵.
        if askp[0] <= 0 and bidp[0] <= 0:
            continue
...
def _str2_ob_subscribe(code: str) -> None:
    """눌림목 트리거: 해당 종목 오더북(H0STASP0) 구독 요청(중복 무시). 실제 전송은 워커."""
    if not STR2_OB_ON_TRIGGER:
        return
    c = str(code).zfill(6)
    with _str2_ob_lock:
        if c in _str2_ob_subs:
            return
        _str2_ob_subs[c] = time.time()
    _str2_ob_cmd_q.put((c, True))


def _str2_ob_unsubscribe(code: str, reason: str = "") -> None:
    """오더북 구독 해제 요청(미구독이면 무시)."""
    c = str(code).zfill(6)
    with _str2_ob_lock:
        if c not in _str2_ob_subs:
            return
        _str2_ob_subs.pop(c, None)
    _str2_ob_latest.pop(c, None)
    _str2_ob_cmd_q.put((c, False))
    if reason:
        logger.info(f"{ts_prefix()} [str2_ob] 해제 {c} ({reason})")


def _str2_ob_pick_acct() -> "str | None":
    """오더북 신규 구독 대상 계좌 선택 — a1 여유 있으면 a1, 만석이면 a2 오버플로.
    ...
    if a1_used < MAX_WSS_SUBSCRIBE:
        return "a1"
    if MULTIACCT_WSS_ENABLED and _a2_active():
...
def _str2_ob_worker() -> None:
    """오더북 구독/해제 전용 워커 — tick 파이프라인·이벤트루프와 분리해 send_request 자기교착 방지.
    구독은 a1 우선·만석 시 a2 오버플로. 해제는 구독했던 계좌(_str2_ob_acct)로 전송."""
    logger.info(f"{ts_prefix()} [str2_ob] 워커 시작")
    while not _stop_event.is_set():
        try:
            c, on = _str2_ob_cmd_q.get(timeout=1.0)
        except queue.Empty:
            continue
        try:
            if on:
                ...
                if kws is None:
                    with _str2_ob_lock:
                        _str2_ob_subs.pop(c, None)
                    continue
                _send_subscribe(kws, req, [c], "1", reason=f"str2_눌림목_오더북({acct})")
                _str2_ob_acct[c] = acct
                ...
                logger.info(f"{ts_prefix()} [str2_ob] 해제 전송 {c}({code_name_map.get(c, c)}) ← {acct}")
        except Exception as e:
            logger.warning(f"{ts_prefix()} [str2_ob] 구독/해제 실패 {c}: {type(e).__name__}: {e}")
...
def _str2_ob_timeout_sweep() -> None:
    ...
    if not _str2_ob_subs:
        return
    now = time.time()
    for c, t0 in list(_str2_ob_subs.items()):
        st = _str2_state.get(c)
        held = bool(st and st.position)
        if (not held) and (now - t0) > STR2_OB_HOLD_SEC:
            _str2_ob_unsubscribe(c, f"{STR2_OB_HOLD_SEC}s 무신호 타임아웃")


def _str2_ob_reset(acct: str) -> None:
    """(재)접속 시 해당 계좌 오더북 구독상태 초기화 → 새 세션엔 구독이 없으므로,
    트리거(미보유 이탈) / 보유중 재구독 가드가 다음 틱에 재구독하도록 로컬 상태를 비운다."""
    if not _str2_ob_acct and not _str2_ob_subs:
        return
    with _str2_ob_lock:
        targets = [c for c, a in _str2_ob_acct.items() if a == acct]
        for c in targets:
            _str2_ob_acct.pop(c, None)
            _str2_ob_subs.pop(c, None)
            _str2_ob_latest.pop(c, None)
    if targets:
        logger.info(f"{ts_prefix()} [str2_ob] {acct} 재접속 → 오더북 {len(targets)}종목 재구독 대기")
...
def _apply_a2_data_subs() -> None:
    """a2 연결의 종목데이터(ccnl/exp) 동적 재구독(재연결 없이 → BB 연속성 유지).
    H0STMKO0(장운영정보)는 _mkstatus_sub_add/remove 경로가 a2 로 직접 처리한다."""
    if not MULTIACCT_WSS_ENABLED:
        return
    with _kws_a2_lock:
        kws2 = _active_kws_a2
    ...
    for req, want in desired.items():
        ...
        to_add = list(want - have)
        if to_del:
            _send_subscribe(kws2, req, to_del, "2", reason="a2_rebuild")
        if to_add:
            _send_subscribe(kws2, req, to_add, "1", reason="a2_rebuild")
        if want:
            _subscribed_a2[req.__name__] = set(want)
        else:
            _subscribed_a2.pop(req.__name__, None)
...
def run_ws_forever_a2():
    ...
    global _ka2, _active_kws_a2, _subscribed_a2
    if not MULTIACCT_WSS_ENABLED:
        return
    if _ka2 is None:
        _ka2 = _setup_ka2()
    ...
    while not _stop_event.is_set():
        ...
        try:
            ...
            # 동적 재구독 추적 초기화(재연결마다 open_map 과 동기화)
            _subscribed_a2 = {req.__name__: set(cs) for req, cs in desired.items() if cs}
        ...
        _str2_ob_reset("a2")   # [260722] a2 종료 → a2 오더북 재구독 대기(a1 여유 or a2 복귀 시)
        try:
            _trigger_ws_rebuild()
        except Exception:
            pass
...
def _fetch_uplimit_band(client, rate_min: str, rate_max: str) -> list[str]:
    ...
    result = []
    for row in out:
        c = str(row.get("stck_shrn_iscd") or row.get("code") or "").strip().zfill(6)
        if not c or c == "000000":
            continue
        if _krx_group_cache and _krx_group_cache.get(c) != "ST":   # 주권만
            continue
        result.append(c)
    return result
...
def _uplimit_watch_loop():
    ...
    try:
        ...
        logger.warning(f"{ts_prefix()} [상한가감시] client 초기화 실패 → 중단: {e}")
        return
    logger.info(f"{ts_prefix()} [상한가감시] 시작 (+{UPLIMIT_WATCH_RATE_MIN}~{UPLIMIT_WATCH_RATE_MAX}% 주권 sticky 편입)")
    while not _stop_event.is_set():
        nowt = datetime.now(KST).time()
        if nowt < dtime(9, 0):
            time.sleep(5.0)
            continue
        if nowt >= dtime(15, 20):     # 마감 임박 → 신규 편입 종료
            logger.info(f"{ts_prefix()} [상한가감시] 15:20 도달 → 신규 편입 종료")
            return
        try:
            cand = _fetch_uplimit_candidates(client)
        except Exception as e:
            logger.warning(f"{ts_prefix()} [상한가감시] 조회 실패: {e}")
            cand = []
        new_codes = []
        with _uplimit_watch_lock:
            for c in cand:
                if c not in _uplimit_watch_codes:
        ...
        if new_codes:
            ...
            _notify(
                ...
                f"{list(zip(new_codes, names))} (누적 {len(_uplimit_watch_codes)})",
                tele=True,
            )
            _trigger_ws_rebuild()
        for _ in range(int(UPLIMIT_WATCH_INTERVAL_SEC * 10)):
            if _stop_event.is_set():
                break
            time.sleep(0.1)
...
if __name__ == "__main__":
    ...
    # [260722] 눌림목 트리거 오더북(H0STASP0) 메인 통합 — 구독/해제 전용 워커(별도 프로세스·버스 불필요)
    if STR2_OB_ON_TRIGGER:
        t_str2_ob = threading.Thread(target=_str2_ob_worker, name="str2-ob", daemon=True)
        t_str2_ob.start()
