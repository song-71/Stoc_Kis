...
def _ccnl_notice_sub_add(code: str) -> None:
    """주문 발생 시 H0STCNI0 체결통보 구독 등록. a2 우선, a1 fallback. tr_key는 htsid."""
    ...
    name = code_name_map.get(code, code)
    # a2 우선
    if _a2_is_connected() and _a2_ccnl_notice_done:
        _ccnl_notice_sub_codes.add(code)
        _ccnl_notice_order_ts[code] = time.time()
        logger.info(f"{ts_prefix()} [체결통보] H0STCNI0 이미 a2 구독됨: {name}({code})")
        return
    # a1 fallback
    with _kws_lock:
        if _active_kws is not None:
            try:
                ...
                _ccnl_notice_sub_codes.add(code)
                _ccnl_notice_order_ts[code] = time.time()
                logger.info(f"{ts_prefix()} [체결통보] H0STCNI0 구독 추가(a1): {name}({code}), tr_key={htsid}")
            except Exception as e:
            ...
            if dtime(8, 29, 59) <= now.time() < dtime(8, 30, 5):
                ...
                if _ccnl_early_add_date != today:
                    ...
                    if _a2_is_connected() and _a2_ccnl_notice_done:
                        ...
                        logger.info(f"{ts_prefix()} [체결통보] 08:29:59 a2 WSS 이미 구독됨 → a1 스킵")
                    else:
                        with _kws_lock:
                            if _active_kws is not None and "ccnl_notice" not in _subscribed:
                                ...
                                if acc_key:
                                    _ccnl_early_add_date = today
                                    _notify(f"{ts_prefix()} [체결통보] 08:29:59 구독 추가(a1): tr_key={acc_key!r}", tele=True)
...
# ── a2 WSS (VI 예상체결가 + 체결통보 + 장운영정보 온디맨드) ────────────
_kws_a2 = None
_kws_a2_ready = threading.Event()
_a2_ccnl_notice_done = False
_a2_mkstatus_codes: set[str] = set()
_a2_mkstatus_ts: dict[str, float] = {}
_A2_MKSTATUS_TIMEOUT = 30.0
_A2_MKSTATUS_MAX = 5
...
def _init_a2_wss() -> None:
    ...
    ka.open_map.update(saved_map)

    def _a2_on_result(ws, tr_id, df, data_info):
        trid = str(tr_id)
        if trid in ("H0STANC0", "H0STOAC0"):
            recv_ts = datetime.now(KST).strftime("%Y-%m-%d %H:%M:%S.%f")
            df.columns = [str(c).strip().lower() for c in df.columns]
            df_pl = pl.from_pandas(df)
            _ingest_queue.put((df_pl, trid, "regular_exp" if trid == "H0STANC0" else "overtime_exp", "N", recv_ts))
        elif trid == "H0STCNI0":
    ...
    logger.info(f"{ts_prefix()} [a2-wss] 연결 시작 (VI 예상체결가 + 체결통보 전용)")
    _kws_a2.start(_a2_on_result)
...
def _vi_exp_sub_add(code: str) -> None:
    """VI 발동 종목에 예상체결가 구독 추가. a2 우선, a1 fallback."""
    ...
    if _a2_is_connected():
        try:
            _kws_a2.send_request(request=exp_ccnl_krx, tr_type="1", data=code)  # noqa: F405
            logger.info(f"{ts_prefix()} [VI감지] 예상체결가 구독 추가(a2): {name}({code})")
            return
        except Exception as e:
            logger.warning(f"{ts_prefix()} [VI감지] a2 exp_ccnl 구독 실패, a1 fallback: {code} {e}")
    # a1 fallback
    with _kws_lock:
        if _active_kws is not None:
            try:
                _send_subscribe(_active_kws, exp_ccnl_krx, [code], "1")  # noqa: F405
                logger.info(f"{ts_prefix()} [VI감지] 예상체결가 구독 추가(a1): {name}({code})")
            except Exception as e:
                logger.warning(f"{ts_prefix()} [VI감지] exp_ccnl 구독 실패: {code} {e}")

def _vi_exp_sub_unsub(code: str) -> None:
    """VI 해제 또는 타임아웃 → 예상체결가 구독 해제. a2 우선, a1 fallback."""
    ...
    if _a2_is_connected():
        try:
            _kws_a2.send_request(request=exp_ccnl_krx, tr_type="2", data=code)  # noqa: F405
            logger.info(f"{ts_prefix()} [VI감지] 예상체결가 구독 해제(a2): {name}({code})")
            return
        except Exception as e:
            logger.warning(f"{ts_prefix()} [VI감지] a2 exp_ccnl 해제 실패, a1 fallback: {code} {e}")
    # a1 fallback
    with _kws_lock:
        if _active_kws is not None:
            try:
                _send_subscribe(_active_kws, exp_ccnl_krx, [code], "2")  # noqa: F405
                logger.info(f"{ts_prefix()} [VI감지] 예상체결가 구독 해제(a1): {name}({code})")
            except Exception as e:
...
def _shutdown(reason: str):
    ...
    _stop_event.set()
    _ws_rebuild_event.set()
    # a2 WSS 종료 (새 데이터 유입 차단)
    if _kws_a2 is not None:
        try:
            _kws_a2.shutdown()
            logger.info("[shutdown] a2 WSS 종료 완료")
        except Exception:
            pass
    # a1 WSS 종료 (새 데이터 유입 차단)
    with _kws_lock:
        if _active_kws is not None:
            _request_ws_close(_active_kws)
            _active_kws = None  # 다른 스레드의 추가 send 방지
    # ── 메모리 데이터 저장 (WSS 종료 후, 큐 소진 대기 → flush → snapshot) ──
    # ingest 큐가 소진될 때까지 최대 8초 대기
...
def run_ws_forever():
    global _close_force_stopped, _active_kws, _last_any_recv_ts
    backoff = 2
    attempt = 0
    ...
    while not _stop_event.is_set():
        ...
        try:
            ...
            ka.open_map.clear()
            _subscribed.clear()
            # H0STCNI0 체결통보: a2 연결 시 a2에서 처리, 아니면 a1
            now_t = datetime.now(KST).time()
            acc_key = _get_ccnl_notice_tr_key()
            if _a2_is_connected() and _a2_ccnl_notice_done:
                logger.info(f"{ts_prefix()} [체결통보] H0STCNI0 a2 WSS에서 구독 중 → a1 스킵")
            elif acc_key:
                ka.KISWebSocket.subscribe(ccnl_notice, [acc_key])  # noqa: F405
                _subscribed["ccnl_notice"] = {acc_key}
                logger.info(f"{ts_prefix()} [체결통보] H0STCNI0 구독 요청(a1): tr_key={acc_key!r}")
            else:
...
if __name__ == "__main__":
    ...
    # a2 WSS 스레드 시작 (VI 예상체결가 + 체결통보 전용)
    if _a2_approval_key:
